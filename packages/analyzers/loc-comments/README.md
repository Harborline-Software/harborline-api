# Harborline.Api.Analyzers.LocComments

Roslyn analyzer that emits **HARBORLINE_API_I18N_001** when a Harborline `.resx` data entry is missing
a translator-context comment.

**Why:** Translators authoring Harborline locales need surrounding context for every string —
...
Harborline project sets `TreatWarningsAsErrors=true` — implicit gating. Promoted to Error
...
```xml
<ItemGroup>
  <PackageReference Include="Harborline.Api.Analyzers.LocComments" Version="..." PrivateAssets="all" />
...
[*.cs]
dotnet_diagnostic.HARBORLINE_API_I18N_001.severity = none
