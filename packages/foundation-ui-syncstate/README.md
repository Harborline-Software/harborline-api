# Harborline.Api.Foundation.UI.SyncState
...
`JsonStringEnumConverter` with `JsonNamingPolicy.CamelCase` produces the lowercase canonical strings — single-word identifiers (`Healthy` / `Stale` / `Offline` / `Conflict` / `Quarantine`) flat-case identically to the canonical wire form. Round-trips through `Harborline.Api.Foundation.Crypto.CanonicalJson.Serialize`.
