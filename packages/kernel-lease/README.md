# Harborline.Api.Kernel.Lease

Flease-inspired distributed lease coordinator for Harborline CP-class record writes.
...
`LEASE_RELEASE` / `LEASE_DENIED` messages defined in `Harborline.Api.Kernel.Sync`
...
```csharp
services
    .AddHarborlineKernelSync()        // registers transport + gossip
    .AddHarborlineKernelLease(
        localNodeId: identity.NodeIdHex,
        localListenEndpoint: "/var/run/harborline-node.sock");
