...
namespace Harborline.Api.Foundation.Transport.Mdns;

/// <summary>
/// Tier-1 (link-local) <see cref="IPeerTransport"/> per ADR 0061 §"Decision".
/// Advertises the local Harborline peer as a mDNS service instance and
...
/// used by <c>Harborline.Api.Kernel.Sync.Discovery.MdnsPeerDiscovery</c>).
/// </para>
/// <para>
...
/// this transport is gated on <c>HARBORLINE_MDNS_TESTS=1</c> per the
...
public sealed class MdnsPeerTransport : IPeerTransport, IAsyncDisposable
{
    ...
    internal bool TryParseAdvertisement(ServiceInstanceDiscoveryEventArgs e, out PeerId peer, out PeerEndpoint endpoint)
    {
        peer = default;
        endpoint = default!;
        ...
        var address = e.Message.AdditionalRecords
            .Concat(e.Message.Answers)
            // Fully qualified: .NET 11 preview.7 added System.Net.AddressRecord, which collides
            // with Makaretu.Dns.AddressRecord under this file's `using System.Net`.
            .OfType<Makaretu.Dns.AddressRecord>()
            .Select(r => r.Address)
            .FirstOrDefault();
