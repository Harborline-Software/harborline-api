...
public enum PolicyRejection
{
    ...
    /// <summary>
    /// The payload exceeds the enforced INLINE-tier ceiling
    /// (<see cref="Models.BlocksDocsOptions.InlineBlobMaxBytes"/>). Distinct from
    /// <see cref="Size"/> (the outer per-attachment hard cap): inline-tier hosts —
    /// e.g. the local node, which stores bytes inline in <c>local-node.db</c> via
    /// <c>StorageRef.Inline</c> — reject above this threshold because no
    /// out-of-line blob tier exists yet (ADR 0127). Additive enum member: hosts
    /// that wire no inline-ceiling gate (the Bridge) never produce this value.
    /// </summary>
    InlineSize,
}
