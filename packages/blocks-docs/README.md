...
Document-attachment substrate for Harborline. Cross-cluster surface — AR invoices, AP bills, leases, inspections, and work orders all attach documents through this cluster. PR 1 ships the entity primitives; PRs 2–6 build the service surface, IBlobStore wiring, and cross-cluster DocumentRef.
...
```bash
dotnet build packages/blocks-docs/Harborline.Blocks.Docs.csproj
dotnet test  packages/blocks-docs/tests/Harborline.Blocks.Docs.Tests.csproj
```
