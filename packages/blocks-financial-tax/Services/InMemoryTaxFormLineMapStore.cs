...
using Harborline.Api.Blocks.FinancialTax.Seeds;
...
public sealed class InMemoryTaxFormLineMapStore : ITaxFormLineMapStore
{
    ...
    public async Task UpsertAsync(TaxFormLineMap map, CancellationToken cancellationToken = default)
    {
        ...
        var now = map.UpdatedAtUtc
            ?? map.CreatedAtUtc
            ?? throw new ArgumentException("Tax-form-map writes require an act instant.", nameof(map));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var seed = DefaultTaxFormLineMap.ScheduleE(chartId, taxYear, createdAtUtc);
