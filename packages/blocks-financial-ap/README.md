...
Accounts-payable substrate for Harborline. Mirror of `blocks-financial-ar`'s shape with vendor-side differences (single AP control account, vendor-supplied bill numbers, optional approval gate, Disputed hold).
...
| `Harborline.Api.Blocks.People.Foundation.Models` | `PartyId` (the vendor FK) |
| `Harborline.Api.Foundation.Assets.Common` | `TenantId`, `Instant` |
| `Harborline.Api.Foundation.Events` | `IDomainEventPublisher` (PR 2's event emission) |
...
```bash
dotnet build packages/blocks-financial-ap/Harborline.Blocks.FinancialAp.csproj
dotnet test  packages/blocks-financial-ap/tests/Harborline.Blocks.FinancialAp.Tests.csproj
```
