# Harborline.Api.Kernel.EventBus

Harborline kernel §3.6 Event Bus — in-process reference implementation using `System.Threading.Channels` with per-entity ordering, Ed25519-signed events, and idempotent delivery.
...
```csharp
services.AddHarborlineKernelEventBus();
services.AddSingleton<IEventHandler<MyEvent>, MyHandler>();
```

## ADR map

- Harborline kernel §3.6 (Event Bus)
- [ADR 0049](../../docs/adrs/0049-audit-trail-substrate.md) — kernel-audit consumes signed event envelopes for the audit trail
...
- `Harborline.Api.Foundation.RuleEngine.EventBridge` — wires this bus to the rule engine for reactive evaluation
