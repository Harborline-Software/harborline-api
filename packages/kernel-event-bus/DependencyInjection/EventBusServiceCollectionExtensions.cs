...
namespace Harborline.Api.Kernel.Events.DependencyInjection;

/// <summary>
/// DI extensions for registering the Harborline kernel event bus (spec §3.6) and event log
/// (paper §2.5, §8).
/// </summary>
public static class EventBusServiceCollectionExtensions
{
    ...
    /// <see cref="Harborline.Api.Foundation.Crypto.IOperationVerifier"/> — the caller
    /// must have already registered one (typically via
    /// <c>AddHarborlineDecentralization</c> or
    ...
    public static IServiceCollection AddHarborlineKernelEventBus(this IServiceCollection services)
    {
    ...
    /// <returns>The same <paramref name="services"/> instance for chaining.</returns>
    public static IServiceCollection AddHarborlineEventLog(
    ...
    /// Intended for tests; call <b>before</b> <see cref="AddHarborlineEventLog"/> (or use it on its
    /// own) so the <c>TryAddSingleton</c> inside <see cref="AddHarborlineEventLog"/> sees an existing
