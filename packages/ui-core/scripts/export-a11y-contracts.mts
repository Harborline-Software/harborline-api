/**
 * Plan 4 Task 1.6 — export Harborline a11y contracts from Storybook stories to JSON.
 *
 * Glob all `src/**\/*.stories.ts`, dynamic-import each via tsx (which we run under),
 * extract `default.parameters.a11y.harborline`, and emit `dist/a11y-contracts.json`
 * keyed by component tag name. The Blazor a11y bridge (Harborline.UIAdapters.Blazor.A11y)
 ...
 *
 * Usage: `pnpm --filter @harborline-software/ui-core build:contracts`
 */
...
interface StoryMetaParameters {
  a11y?: {
    harborline?: Record<string, unknown>;
  };
}
...
async function main() {
  ...
  for (const file of stories) {
    ...
    const harborline = meta.parameters?.a11y?.harborline;
    if (!harborline) {
      skipped.push({ file: relative(SRC_ROOT, file), reason: 'no parameters.a11y.harborline' });
      continue;
    }
...
function deriveTagFromFilename(file: string): string | null {
  // harborline-button.stories.ts → harborline-button
