 ...
 * then enforces Harborline-specific contract assertions declared in
 * `parameters.a11y.harborline` per ADR 0034. Pilot components (harborline-button,
 * harborline-dialog, harborline-syncstate-indicator) are treated as smoke gates — any
...
const PILOT_COMPONENT_TAGS = new Set([
  'harborline-button',
  'harborline-dialog',
  'harborline-syncstate-indicator',
]);
...
const A11Y_THEME = parseA11yTheme(process.env.STORYBOOK_A11Y_THEME);

function parseA11yTheme(value: string | undefined): A11yTheme {
  if (value === undefined || value === 'light') return 'light';
  if (value === 'dark') return 'dark';
  throw new Error(`STORYBOOK_A11Y_THEME must be 'light' or 'dark', received '${value}'.`);
}

/**
 * Harborline a11y contract block as authored on each story's `parameters.a11y.harborline`.
 * The .NET-side mirror of this record is not realised in this repository yet; there is no
 * Blazor a11y adapter package at this pin.
 */
interface HarborlineContract {
  ariaPattern?: string;
...
const config: TestRunnerConfig = {
  async preVisit(page) {
    // Reset any per-page state Harborline components depend on and pin the
    // explicit token switch for this matrix pass. The OS preference remains
    // irrelevant because data-theme wins in tokens.css.
  ...
  async postVisit(page, context) {
    ...
    await runAxe(page, componentTag, A11Y_THEME);

    // 2. Run Harborline-specific contract assertions if declared.
    const harborline = (storyContext.parameters as { a11y?: { harborline?: HarborlineContract } } | undefined)
      ?.a11y?.harborline;
    if (harborline) {
      await runHarborlineAssertions(page, harborline, componentTag);
    }
  },
};
...
async function runAxe(
  ...
  const heading = isPilot
    ? `Pilot component '${componentTag}' a11y SMOKE GATE FAILURE [theme=${theme}] — fails the whole run.`
    : `A11y violations in '${componentTag ?? 'unknown'}' [theme=${theme}]`;

  throw new Error(`${heading}\n${message}`);
}

async function runHarborlineAssertions(
  page: Page,
  contract: HarborlineContract,
  componentTag: string | undefined,
): Promise<void> {
  ...
  if (contract.focus?.initial && contract.focus.initial !== 'self' && contract.focus.initial !== 'none') {
    const focusInsideRoot = await page.evaluate(() => {
      ...
      // adopts a "data-harborline-focus-target" sentinel.
...
async function assertKeyboardMap(
  ...
  // exposing `data-harborline-keyboard-map="enabled"` on a descendant of #storybook-root.
  // Components that haven't adopted the data-harborline-fired convention yet are skipped
  ...
  const enabled = await page.evaluate(() =>
    document.querySelector('#storybook-root [data-harborline-keyboard-map="enabled"]') !== null,
  );

  if (!enabled) {
    // eslint-disable-next-line no-console
    console.warn(
      `[${componentTag}] skipping keyboardMap enforcement: component has not opted in via data-harborline-keyboard-map="enabled".`,
    );
    return;
  }

  for (const binding of bindings) {
    // Reset the host's data-harborline-fired before each binding.
    await page.evaluate(() => {
      const host = document.querySelector('#storybook-root');
      if (host) host.setAttribute('data-harborline-fired', '');
    });
    ...
    const fired = await page.evaluate(() =>
      document.querySelector('#storybook-root')?.getAttribute('data-harborline-fired') ?? '',
    );

    if (!fired.includes(binding.action)) {
      throw new Error(
        `[${componentTag}] keyboardMap '${chord}' was expected to fire '${binding.action}'; data-harborline-fired='${fired}'.`,
      );
    }
  }
}
...
async function assertDirectionalIconsMirroredInRtl(
  ...
  // matching `[data-harborline-direction="<state>"]` for each state in the contract's
  ...
  const enabled = await page.evaluate(() =>
    document.querySelector('#storybook-root [data-harborline-direction]') !== null,
  );

  if (!enabled) {
    // eslint-disable-next-line no-console
    console.warn(
      `[${componentTag}] skipping directionalIcons enforcement: component has not opted in via [data-harborline-direction="<state>"] sentinel.`,
    );
    return;
  }
  ...
  for (const state of directionalStates) {
    const selector = `#storybook-root [data-harborline-direction="${state}"]`;
