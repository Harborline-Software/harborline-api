...
public static class IntegrationAtlasServiceCollectionExtensions
{
    ...
    /// call <c>AddHarborlineRecoveryCoordinator()</c> (in
    ...
    /// implementation
    /// (<c>Harborline.Api.Foundation.Recovery.Crypto.TenantKeyDecryptCapabilityProvider</c>)
    /// transitively depends on
    /// <c>Harborline.Api.Foundation.Recovery.TenantKey.ITenantKeyProvider</c>.
    ...
    /// <c>HARBORLINE_API_INTEGRATION_AUDIT001</c> typed-payload
    /// requirement.
    /// </para>
    /// </remarks>
    ...
    /// registered. Call <c>AddHarborlineRecoveryCoordinator()</c>
    /// first.
    /// </exception>
    public static IServiceCollection AddHarborlineIntegrationAtlas(
        this IServiceCollection services)
    {
        ...
        if (!services.Any(d => d.ServiceType == typeof(IDecryptCapabilityProvider)))
        {
            throw new InvalidOperationException(
                "AddHarborlineRecoveryCoordinator() must be called before "
                + "AddHarborlineIntegrationAtlas(). IDecryptCapabilityProvider is "
