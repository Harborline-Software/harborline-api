...
///   <item><c>roster</c> — immutable roster-root id (for parallel-roster segregation).</item>
...
public sealed class MdnsPeerDiscovery : IPeerDiscovery
{
    ...
    private void OnServiceInstanceDiscovered(object? sender, ServiceInstanceDiscoveryEventArgs e)
    {
        try
        {
            ...
            var tracker = _known.AddOrUpdate(
                ad.NodeId,
                _ => new PeerTracker(ad, _timeProvider.GetUtcNow()),
                (_, existing) => existing.TouchedWith(ad, _timeProvider.GetUtcNow()));
    ...
    private static void PopulateTxt(ServiceProfile profile, PeerAdvertisement self)
    {
        ...
        profile.AddProperty("roster", self.RosterId);
    ...
    private PeerAdvertisement? ParseAdvertisement(ServiceInstanceDiscoveryEventArgs e)
    {
        ...
        foreach (var entry in strings)
        {
            ...
            switch (key)
            {
                ...
                case "roster": rosterId = value; break;
    ...
    {
        if (_options.FilterByRosterId
            && _self is not null
            && !string.Equals(_self.RosterId, ad.RosterId, StringComparison.Ordinal))
        {
            return false;
        }
