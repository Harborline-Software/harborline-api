...
/// TTL on a peer record, roster-scoped filtering. The service-type string
/// (<c>_sunfish-node._tcp.local</c>) remains fixed in sync-daemon-protocol
/// spec §3.1 so peers on different machines can find one another; the immutable
/// roster root, rather than an environment label or service-type variant, scopes discovery.
/// </remarks>
public sealed class PeerDiscoveryOptions
{
    ...
    /// <summary>
    /// When <c>true</c>, advertisements from a different immutable roster root are suppressed before
    /// <see cref="IPeerDiscovery.PeerDiscovered"/> fires. Default <c>true</c>.
    /// </summary>
    public bool FilterByRosterId { get; set; } = true;
}
