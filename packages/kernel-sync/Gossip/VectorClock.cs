...
public sealed class VectorClock
{
    ...
    /// <summary>Set or advance the entry for the given node id without permitting rollback.</summary>
    public void Set(string nodeId, ulong sequence)
    {
        ArgumentException.ThrowIfNullOrEmpty(nodeId);
        lock (_lock)
        {
            if (_entries.TryGetValue(nodeId, out var current) && sequence < current)
            {
                throw new InvalidOperationException(
                    $"Vector-clock entry for '{nodeId}' cannot move backwards from {current} to {sequence}.");
            }
