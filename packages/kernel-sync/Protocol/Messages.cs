...
internal static class MessageTypes
{
    ...
    /// <summary>
    /// PRE-TRUST enrollment request (#1301 F-1). Sent by a JOINING peer as the
    /// FIRST frame on the 7473 sync listener — BEFORE the trusted HELLO — so a
    /// remote, not-yet-trusted node can present an invite over the network.
    /// kernel-sync carries the request payload as OPAQUE bytes: the enrollment
    /// PROTOCOL (foundation-identity-atlas <c>WireEnrollment</c>) lives above this
    /// package and is not visible here, so the transport stays protocol-agnostic.
    /// </summary>
    public const string EnrollRequest = "ENROLL_REQUEST";

    /// <summary>
    /// PRE-TRUST enrollment response (#1301 F-1). A's reply to an
    /// <see cref="EnrollRequest"/> on the network listener: either the opaque
    /// bootstrap payload (accepted) or a fail-closed rejection. Carried as opaque
    /// bytes for the same protocol-agnostic reason.
    /// </summary>
    public const string EnrollResponse = "ENROLL_RESPONSE";
}
...
{
    /// <summary>Create a legacy-shape proposal without named capabilities.</summary>
    public CapabilityNegMessage(
        IReadOnlyList<string> ProposedStreams,
        IReadOnlyList<string> CpLeases,
        IReadOnlyList<BucketRequest> BucketSubscriptions)
        : this(ProposedStreams, CpLeases, BucketSubscriptions, Array.Empty<string>())
    {
    }
    ...
    public byte[] ToCbor()
    {
        ...
        writer.WriteStartMap(Capabilities.Count > 0 ? 4 : 3);
        ...
        {
            writer.WriteTextString("capabilities");
            writer.WriteStartArray(Capabilities.Count);
        ...
        while (reader.PeekState() != CborReaderState.EndMap)
        {
            ...
            switch (key)
            {
                ...
                case "bucket_subscriptions":
                    ...
                    break;
                case "capabilities":
                    capabilities = ReadStringArray(reader);
                    break;
                default:
                    reader.SkipValue();
                    break;
...
{
    /// <summary>The named protocol capabilities granted by the responder.</summary>
    public IReadOnlyList<string> GrantedCapabilities { get; init; } = Array.Empty<string>();

    /// <summary>Create an acknowledgement that grants named protocol capabilities.</summary>
    public AckMessage(
        IReadOnlyList<string> GrantedSubscriptions,
        IReadOnlyList<Rejection> Rejected,
        IReadOnlyList<string> GrantedCapabilities)
        : this(GrantedSubscriptions, Rejected)
    {
    ...
    public byte[] ToCbor()
    {
        ...
        writer.WriteStartMap(GrantedCapabilities.Count > 0 ? 5 : 4);
        ...
        if (GrantedCapabilities.Count > 0)
        {
            writer.WriteTextString("granted_capabilities");
            writer.WriteStartArray(GrantedCapabilities.Count);
        ...
        while (reader.PeekState() != CborReaderState.EndMap)
        {
            ...
            switch (key)
            {
                ...
                case "max_deltas_per_sec":
                    ...
                    break;
                case "granted_capabilities":
                    var capabilityCount = reader.ReadStartArray();
                    capabilities = new List<string>(capabilityCount ?? 0);
                    while (reader.PeekState() != CborReaderState.EndArray)
                    {
                        capabilities.Add(reader.ReadTextString());
                    }
                    reader.ReadEndArray();
                    break;
                default:
...
public sealed record GossipPingMessage(
    ...
    ulong MonotonicNonce,
    IReadOnlyDictionary<string, byte[]> StateVectors)
{
    /// <summary>Create a legacy-shape ping without CRDT state vectors.</summary>
    public GossipPingMessage(
        IReadOnlyDictionary<string, ulong> VectorClock,
        MembershipDelta PeerMembershipDelta,
    ...
    public byte[] ToCbor()
    {
        ...
        writer.WriteStartMap(StateVectors.Count > 0 ? 4 : 3);
        ...
        {
            writer.WriteTextString("state_vectors");
            writer.WriteStartMap(StateVectors.Count);
            foreach (var (documentId, stateVector) in StateVectors)
            {
                writer.WriteTextString(documentId);
                writer.WriteByteString(stateVector);
            }
            writer.WriteEndMap();
        }
    ...
    public static GossipPingMessage FromCbor(ReadOnlySpan<byte> cbor)
    {
        ...
        while (reader.PeekState() != CborReaderState.EndMap)
        {
            ...
            switch (key)
            {
                ...
                case "monotonic_nonce":
                    ...
                    break;
                case "state_vectors":
                    var stateVectorCount = reader.ReadStartMap();
                    stateVectors = new Dictionary<string, byte[]>(stateVectorCount ?? 0, StringComparer.Ordinal);
                    while (reader.PeekState() != CborReaderState.EndMap)
                    {
                        stateVectors[reader.ReadTextString()] = reader.ReadByteString();
                    }
                    reader.ReadEndMap();
                    break;
                default:
        ...
        return new GossipPingMessage(clock, delta, nonce, stateVectors);
    }
}
...
public enum ErrorCode
{
    ...
    ClassNotGranted,
    ExpiredAttestation,
    DeltaStreamDeadlineExceeded,

    /// <summary>
    /// The peer's HELLO is cryptographically valid (it owns the key it
    /// presented) but that key is not trusted — it does not chain to the
    /// shared root identity / trusted-peer set. Multi-device shared-root TOFU
    /// (sync-daemon-protocol §4; INC-3). Non-recoverable — fail-closed.
    /// </summary>
    PeerUntrusted,
}
...
public static class ErrorCodes
{
    public static string ToWire(ErrorCode code) => code switch
    {
        ...
        ErrorCode.DeltaStreamDeadlineExceeded => "DELTA_STREAM_DEADLINE_EXCEEDED",
        ErrorCode.PeerUntrusted => "PEER_UNTRUSTED",
        _ => "UNKNOWN",
    };
    ...
    public static ErrorCode FromWire(string wire) => wire switch
    {
        ...
        "DELTA_STREAM_DEADLINE_EXCEEDED" => ErrorCode.DeltaStreamDeadlineExceeded,
        "PEER_UNTRUSTED" => ErrorCode.PeerUntrusted,
        _ => ErrorCode.Unknown,
    };
}
...
/// <summary>
/// PRE-TRUST ENROLLMENT REQUEST (#1301 F-1) — the FIRST frame a JOINING peer may send on the 7473 sync listener,
/// BEFORE the trusted HELLO. It carries the enrollment-protocol request as an OPAQUE byte payload: kernel-sync
/// does not know the payload's structure (the protocol — foundation-identity-atlas <c>WireEnrollment</c> — sits
/// ABOVE this package), so the transport stays protocol-agnostic. The accept loop hands the payload to the
/// injected <c>IPreTrustEnrollmentHandler</c>, which decodes + validates it. AUTH is by the INVITE the payload
/// carries (single-use / roster-admin-signed / TTL), NOT the trusted-session gate and NOT roster membership — so
/// a remote, not-yet-trusted B can present it. An invalid/missing/expired/replayed invite is rejected fail-closed
/// with no roster/genesis leak (the handler returns a bare <see cref="EnrollResponseMessage"/> reject).
/// </summary>
public sealed record EnrollRequestMessage(byte[] Payload)
{
    public byte[] ToCbor()
    {
        var writer = CborEnvelope.OpenEnvelope(MessageTypes.EnrollRequest);
        writer.WriteStartMap(1);
        writer.WriteTextString("payload");
        writer.WriteByteString(Payload ?? Array.Empty<byte>());
        writer.WriteEndMap();
        return CborEnvelope.CloseEnvelope(writer);
    }

    public static EnrollRequestMessage FromCbor(ReadOnlySpan<byte> cbor)
    {
        var reader = CborEnvelope.OpenBody(cbor, MessageTypes.EnrollRequest);
        reader.ReadStartMap();
        byte[]? payload = null;
        while (reader.PeekState() != CborReaderState.EndMap)
        {
            var key = reader.ReadTextString();
            if (key == "payload") payload = reader.ReadByteString();
            else reader.SkipValue();
        }
        reader.ReadEndMap();
        if (payload is null)
        {
            throw new CborContentException("ENROLL_REQUEST body missing required 'payload' field.");
        }
        return new EnrollRequestMessage(payload);
    }
}

/// <summary>
/// PRE-TRUST ENROLLMENT RESPONSE (#1301 F-1) — A's reply to an <see cref="EnrollRequestMessage"/> on the network
/// listener. On <see cref="Accepted"/> it carries the OPAQUE bootstrap payload (which the joiner's
/// enrollment-protocol client decodes + validates against its out-of-band anchor); on rejection it carries an
/// empty payload + an opaque <see cref="RejectReason"/> token (deliberately coarse — it never discloses WHY in a
/// way that leaks roster/genesis state, only that the invite did not pass; the joiner treats any reject as
/// "adopt nothing"). kernel-sync stays protocol-agnostic: the payload is bytes it neither builds nor inspects.
/// </summary>
public sealed record EnrollResponseMessage(bool Accepted, byte[] Payload, string? RejectReason)
{
    public byte[] ToCbor()
    {
        var writer = CborEnvelope.OpenEnvelope(MessageTypes.EnrollResponse);
        writer.WriteStartMap(3);
        writer.WriteTextString("accepted");
        writer.WriteBoolean(Accepted);
        writer.WriteTextString("payload");
        writer.WriteByteString(Payload ?? Array.Empty<byte>());
        writer.WriteTextString("reject_reason");
        writer.WriteTextString(RejectReason ?? string.Empty);
        writer.WriteEndMap();
        return CborEnvelope.CloseEnvelope(writer);
    }

    public static EnrollResponseMessage FromCbor(ReadOnlySpan<byte> cbor)
    {
        var reader = CborEnvelope.OpenBody(cbor, MessageTypes.EnrollResponse);
        reader.ReadStartMap();
        bool accepted = false;
        ...
        while (reader.PeekState() != CborReaderState.EndMap)
        {
            ...
            switch (key)
            {
                case "accepted": accepted = reader.ReadBoolean(); break;
                case "payload": payload = reader.ReadByteString(); break;
                case "reject_reason":
                    var r = reader.ReadTextString();
                    rejectReason = string.IsNullOrEmpty(r) ? null : r;
                    break;
                default: reader.SkipValue(); break;
            }
        }
        reader.ReadEndMap();
        return new EnrollResponseMessage(accepted, payload, rejectReason);
    }
}
...
public static class SyncMessageCodec
{
    public static object Decode(ReadOnlySpan<byte> cbor)
    {
        ...
        return type switch
        {
            ...
            MessageTypes.EnrollRequest => EnrollRequestMessage.FromCbor(cbor),
            MessageTypes.EnrollResponse => EnrollResponseMessage.FromCbor(cbor),
    ...
    public static byte[] Encode(object message)
    {
        return message switch
        {
            ...
            EnrollRequestMessage m => m.ToCbor(),
            EnrollResponseMessage m => m.ToCbor(),
