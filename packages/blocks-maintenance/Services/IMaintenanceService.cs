...
public interface IMaintenanceService
{
    ...
    /// <summary>
    /// Archives the vendor with the specified <paramref name="id"/> per ADR 0108
    /// (<c>IArchivable</c>). Vendor has no separate archive column — the archived
    /// state is PROJECTED onto the existing status: this sets
    /// <see cref="Models.Vendor.Status"/> to <see cref="Models.VendorStatus.Inactive"/>
    /// and emits <see cref="Harborline.Api.Kernel.Audit.AuditEventType.VendorArchived"/>.
    /// </summary>
    /// <remarks>
    /// Archival is recoverable: an archived vendor is excluded from default
    /// <see cref="ListVendorsAsync"/> results (unless
    /// <see cref="ListVendorsQuery.IncludeArchived"/> is set) but remains resolvable by id
    /// via <see cref="GetVendorAsync"/> for referential history, and existing references to
    /// the vendor (e.g. from work orders) are NOT invalidated.
    /// </remarks>
    /// <param name="id">The vendor to archive.</param>
    /// <param name="actor">The actor performing the archival (for audit attribution).</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>The archived vendor.</returns>
    /// <exception cref="InvalidOperationException">
    /// Thrown when no vendor with <paramref name="id"/> exists in the current tenant.
    /// </exception>
    ValueTask<Vendor> ArchiveVendorAsync(VendorId id, ActorId actor, CancellationToken ct = default);
    ...
    /// (ADR 0054) and is referenced via <see cref="Harborline.Api.Foundation.Integrations.Signatures.SignatureEventRef"/>.
