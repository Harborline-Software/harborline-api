...
using Harborline.Api.Blocks.Maintenance.Audit;
...
public sealed class InMemoryMaintenanceService : IMaintenanceService
{
    ...
    private readonly Harborline.Api.Foundation.Integrations.Messaging.IThreadStore? _threadStore;
    private readonly Harborline.Api.Foundation.Integrations.Payments.IPaymentGateway? _paymentGateway;
    private readonly Harborline.Api.Foundation.Integrations.Signatures.ISignatureCapture? _signatureCapture;
    ...
    // paths kept for test-rig back-compat. In production (Harborline MauiProgram
    ...
    /// <see cref="InMemoryMaintenanceService(ITenantContext, TimeProvider)"/> overload or
    /// one of the audit-emission variants below.
    /// </summary>
    public InMemoryMaintenanceService(TimeProvider timeProvider)
    {
    ...
    /// </summary>
    public InMemoryMaintenanceService(ITenantContext tenantContext, TimeProvider timeProvider)
    {
    ...
    public InMemoryMaintenanceService(
        IAuditTrail auditTrail, IOperationSigner signer, TenantId tenantId, TimeProvider timeProvider)
    {
    ...
    /// audit-tenant overload <see cref="InMemoryMaintenanceService(ITenantContext, IAuditTrail, IOperationSigner, TenantId, TimeProvider)"/>.
    /// </summary>
    public InMemoryMaintenanceService(
    ...
    /// <see cref="Harborline.Api.Foundation.Integrations.Messaging.IThreadStore"/>
    /// (creates a coordination thread on
    /// <see cref="CreateWorkOrderAsync"/>),
    /// <see cref="Harborline.Api.Foundation.Integrations.Payments.IPaymentGateway"/>
    /// (authorize on <see cref="WorkOrderStatus.Invoiced"/>; capture on
    /// <see cref="WorkOrderStatus.Paid"/>), and
    /// <see cref="Harborline.Api.Foundation.Integrations.Signatures.ISignatureCapture"/>
    ...
    public InMemoryMaintenanceService(
        ...
        IOperationSigner signer,
        TenantId tenantId,
        Harborline.Api.Foundation.Integrations.Messaging.IThreadStore? threadStore,
        Harborline.Api.Foundation.Integrations.Payments.IPaymentGateway? paymentGateway,
        Harborline.Api.Foundation.Integrations.Signatures.ISignatureCapture? signatureCapture,
        TimeProvider timeProvider)
        : this(auditTrail, signer, tenantId, timeProvider)
    {
        ...
        CancellationToken ct)
    {
        ...
        await EmitAsync(AuditEventType.TenantBoundaryViolation, payload, occurredAt, ct).ConfigureAwait(false);
    }
    ...
    public async ValueTask<Vendor?> GetVendorAsync(VendorId id, CancellationToken ct = default)
    {
        ...
        if (vendor is not null && CurrentTenantId is { } tenant && !vendor.TenantId.Equals(tenant))
        {
            await EmitTenantBoundaryViolationAsync(
                "Vendor", id.Value, _timeProvider.GetUtcNow(), ct).ConfigureAwait(false);
            return null;
        }
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        foreach (var vendor in _vendors.Values)
        {
            ...
            // ADR 0108 default-list exclusion — archived vendors are hidden
            // unless the caller opts in. For Vendor the IArchivable archived
            // state is PROJECTED onto VendorStatus.Inactive (no separate
            // ArchivedAt column); this is a second WHERE predicate at the same
            // seam as the tenant filter, keyed on the existing status.
            if (vendor.Status == VendorStatus.Inactive && !query.IncludeArchived)
                continue;
    ...
    /// <inheritdoc />
    public async ValueTask<Vendor> ArchiveVendorAsync(VendorId id, ActorId actor, CancellationToken ct = default)
    {
        ct.ThrowIfCancellationRequested();

        // Tenant-guarded resolve: GetVendorAsync returns null (and emits a
        // TenantBoundaryViolation) on a cross-tenant id, so archival cannot
        // reach across tenants. Absent / cross-tenant → uniform not-found.
        var vendor = await GetVendorAsync(id, ct).ConfigureAwait(false);
        if (vendor is null)
            throw new InvalidOperationException($"Vendor '{id.Value}' not found.");

        // ADR 0108 archive: project the IArchivable archived state onto the
        // existing VendorStatus.Inactive (no separate ArchivedAt column —
        // boundary-adapter discipline, no rip-and-replace migration).
        // Recoverable; does NOT block existing references.
        var archived = vendor with { Status = VendorStatus.Inactive };
        _vendors[id] = archived;

        await EmitAsync(
            AuditEventType.VendorArchived,
            VendorAuditPayloadFactory.VendorArchived(id, actor),
    ...
    public ValueTask<MaintenanceRequest> SubmitRequestAsync(SubmitMaintenanceRequest request, CancellationToken ct = default)
    {
        ...
        var req = new MaintenanceRequest(
            ...
            CreatedAtUtc: new Instant(_timeProvider.GetUtcNow()));
    ...
    public ValueTask<Rfq> SendRfqAsync(SendRfqRequest request, CancellationToken ct = default)
    {
        ...
        var rfq = new Rfq(
            ...
            SentAtUtc: new Instant(_timeProvider.GetUtcNow()));
    ...
    public ValueTask<Quote> SubmitQuoteAsync(SubmitQuoteRequest request, CancellationToken ct = default)
    {
        ...
        var quote = new Quote(
            ...
            SubmittedAtUtc: new Instant(_timeProvider.GetUtcNow()));
    ...
    public async ValueTask<Quote> AcceptQuoteAsync(QuoteId id, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var workOrder = new WorkOrder
            {
                ...
                ScheduledDate = DateOnly.FromDateTime(_timeProvider.GetUtcNow().UtcDateTime.AddDays(7)), // placeholder; caller can update
                CompletedDate = null,
                EstimatedCost = new Harborline.Api.Foundation.Integrations.Payments.Money(target.Amount, Harborline.Api.Foundation.Integrations.Payments.CurrencyCode.USD),
                TotalCost = null,
    ...
    public async ValueTask<WorkOrder> CreateWorkOrderAsync(CreateWorkOrderRequest request, CancellationToken ct = default)
    {
        ...
        Harborline.Api.Foundation.Integrations.Messaging.ThreadId? primaryThread = null;
        if (request.CreateThread && _threadStore is not null)
        {
            _vendors.TryGetValue(request.AssignedVendorId, out var vendor);
            var operatorParticipant = new Harborline.Api.Foundation.Integrations.Messaging.Participant
            {
                Id = new Harborline.Api.Foundation.Integrations.Messaging.ParticipantId(Guid.NewGuid()),
                Identity = ActorId.System,
                DisplayName = "Operator",
            };
            var vendorParticipant = new Harborline.Api.Foundation.Integrations.Messaging.Participant
            {
                Id = new Harborline.Api.Foundation.Integrations.Messaging.ParticipantId(Guid.NewGuid()),
            ...
            primaryThread = await _threadStore.CreateAsync(
                ...
                Harborline.Api.Foundation.Integrations.Messaging.MessageVisibility.Public,
                ct).ConfigureAwait(false);
        }
        ...
        var workOrder = new WorkOrder
        {
            ...
            Notes = request.Notes,
            CreatedAtUtc = new Instant(occurredAt),
            ...
            if (_paymentGateway is not null && updated.EstimatedCost is not null)
            {
                if (newStatus == WorkOrderStatus.Invoiced)
                {
                    var auth = await _paymentGateway.AuthorizeAsync(
                        new Harborline.Api.Foundation.Integrations.Payments.PaymentAuthorizationRequest(
                            updated.Tenant,
    ...
    public async ValueTask<WorkOrder?> GetWorkOrderAsync(WorkOrderId id, CancellationToken ct = default)
    {
        ...
        {
            await EmitTenantBoundaryViolationAsync(
                "WorkOrder", id.Value, _timeProvider.GetUtcNow(), ct).ConfigureAwait(false);
            return null;
        }
