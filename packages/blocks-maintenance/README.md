# Harborline.Api.Blocks.Maintenance
...
DI registration is currently caller-driven (the block has no `AddHarborline*` extension yet); each service is registered directly. Producers of new services follow the same pattern.
