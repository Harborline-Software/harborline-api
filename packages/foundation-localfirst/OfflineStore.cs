...
public interface IOfflineStore
{
    ...
    /// <summary>
    /// Returns a snapshot of keys starting with the given prefix, ordered by
    /// <see cref="StringComparer.Ordinal"/> ascending. The Ordinal, culture-invariant
    /// ordering is a CONTRACT guarantee — not an implementation accident: callers that
    /// depend on a stable, deterministic key sequence (e.g. the deterministic export
    /// slice) may rely on it without re-sorting. Every <see cref="IOfflineStore"/>
    /// implementation must honour it; the guarantee is pinned by the interface-level
    /// contract test (OfflineStoreContractTests).
    /// </summary>
