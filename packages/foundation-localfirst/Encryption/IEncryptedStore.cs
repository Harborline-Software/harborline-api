...
public interface IEncryptedStore
{
    ...
    /// <summary>
    /// Atomically writes <paramref name="value"/> only when the current value is byte-equal to
    /// <paramref name="expectedValue"/>. A null expectation means the key must not exist.
    /// </summary>
    /// <returns>True when the conditional write committed; false when the expectation was stale.</returns>
    /// <remarks>
    /// Implementations that cannot provide one-statement conditional atomicity must fail closed.
    /// This primitive supports versioned single-document authorities; it is not a multi-key transaction.
    /// </remarks>
    Task<bool> CompareExchangeAsync(
        ...
        throw new NotSupportedException(
            "This encrypted store implementation does not support atomic compare-exchange.");
    ...
    /// <c>Harborline.Api.Foundation.Recovery.IRecoveryCoordinator</c>. After a
