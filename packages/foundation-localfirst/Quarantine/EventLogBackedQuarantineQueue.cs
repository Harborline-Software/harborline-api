...
namespace Harborline.Api.Foundation.LocalFirst.Quarantine;
...
public sealed class EventLogBackedQuarantineQueue : IQuarantineQueue
{
    ...
    /// </summary>
    public EventLogBackedQuarantineQueue(IEventLog log, TimeProvider timeProvider)
    {
    ...
    public async Task<Guid> EnqueueAsync(QuarantineEntry entry, CancellationToken ct)
    {
        ...
        await AppendEventAsync(id, KindEnqueued, envelope, entry.QueuedAt, ct).ConfigureAwait(false);
    ...
    public async Task PromoteAsync(Guid entryId, string actor, CancellationToken ct)
    {
        ...
        await AppendEventAsync(entryId, KindPromoted, envelope, timestamp, ct).ConfigureAwait(false);
    ...
    public async Task RejectAsync(Guid entryId, string actor, string reason, CancellationToken ct)
    {
        ...
        await AppendEventAsync(entryId, KindRejected, envelope, timestamp, ct).ConfigureAwait(false);
    ...
    private Task AppendEventAsync<T>(
        Guid entryId, string kind, T envelope, DateTimeOffset at, CancellationToken ct)
    {
