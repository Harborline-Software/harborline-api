...
public sealed class InMemoryQuarantineQueue : IQuarantineQueue
{
    ...
    /// <summary>Creates the queue with the process root clock for status-transition acts.</summary>
    public InMemoryQuarantineQueue(TimeProvider timeProvider) =>
    ...
    public Task PromoteAsync(Guid entryId, string actor, CancellationToken ct)
    {
        ...
        TransitionFromPending(
            ...
            current => current with
            {
                ...
                StatusChangedAt = _timeProvider.GetUtcNow(),
