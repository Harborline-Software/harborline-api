...
namespace Harborline.Api.Blocks.People.Foundation.Services;
...
/// <see cref="Harborline.Api.Foundation.Capabilities.Principal"/>, not a
...
public interface IPartyWriteService
{
    ...
    /// <param name="kind">The party kind.</param>
    /// <param name="displayName">Human-readable name.</param>
    /// <param name="actor">The party performing the write, for provenance.</param>
    /// <param name="at">The admitted instant for the create act and its event.</param>
    /// <param name="id">
    /// Optional explicit party id, symmetric with <c>Party.Create</c>, which has always accepted one.
    /// Needed to ADOPT an id that already exists in another plane rather than mint a fresh one — the
    /// founder attach binds to the signed roster's genesis party id, because the invitation issuer and
    /// the roster granter authority both resolve a party id IN THE ROSTER. Omit it for ordinary
    /// creation and a fresh id is minted exactly as before.
    /// </param>
    ...
    /// </summary>
    Task<Party> UpdateAsync(Party updated, PartyId actor, DateTimeOffset at, CancellationToken cancellationToken = default);

    /// <summary>Tombstone a Party (sets <c>DeletedAt</c> + <c>DeletedReason</c>; never hard-deleted).</summary>
    Task<Party> DeleteAsync(PartyId id, string? reason, PartyId actor, DateTimeOffset at, CancellationToken cancellationToken = default);
