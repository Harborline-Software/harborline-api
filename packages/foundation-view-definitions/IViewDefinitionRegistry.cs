...
/// <summary>Registers and reads immutable, descriptor-admitted view definition revisions.</summary>
public interface IViewDefinitionRegistry
{
    /// <summary>
    ...
    /// content raises <see cref="ViewDefinitionGovernanceException"/>.
    /// </summary>
    ...
    /// <returns>The stored canonical view definition.</returns>
    ValueTask<ViewDefinition> RegisterAsync(
    ...
    ValueTask<ViewDefinition?> GetDefinitionAsync(
        string tenant,
        string key,
        string version,
        CancellationToken cancellationToken = default);
    ...
    ValueTask<IReadOnlyList<ViewDefinition>> ListDefinitionsAsync(
        string tenant,
        CancellationToken cancellationToken = default);
    ...
    ValueTask<ViewDefinitionVersionList?> ListVersionsAsync(
        string tenant,
        string key,
        CancellationToken cancellationToken = default);
...
/// <summary>Raised when view-definition content violates registry-owned governance.</summary>
public sealed class ViewDefinitionGovernanceException : InvalidOperationException
{
    ...
    public ViewDefinitionGovernanceException(string errorCode)
        : base($"View definition governance refused: {errorCode}.")
    {
...
public sealed record ViewDefinitionVersionList(
    string Ordering,
    IReadOnlyList<ViewDefinition> Versions);
