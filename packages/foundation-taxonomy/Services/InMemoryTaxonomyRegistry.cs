...
using Harborline.Api.Foundation.Taxonomy.Audit;
...
public sealed class InMemoryTaxonomyRegistry : ITaxonomyRegistry
{
    ...
    public InMemoryTaxonomyRegistry(IAuditTrail auditTrail, IOperationSigner signer, TimeProvider time)
    {
        ...
        CancellationToken ct)
    {
        ...
        await EmitAsync(tenantId, AuditEventType.TaxonomyDefinitionCreated, TaxonomyAuditPayloadFactory.Created(def, owner), at, ct).ConfigureAwait(false);
        return def;
    }
        ...
        CancellationToken ct)
    {
        ...
        await EmitAsync(tenantId, AuditEventType.TaxonomyVersionPublished, TaxonomyAuditPayloadFactory.VersionPublished(id, newVersion, publishedBy), at, ct).ConfigureAwait(false);
        return newDef;
    }
        ...
        CancellationToken ct)
    {
        ...
        var at = _time.GetUtcNow();
        var retired = def with { RetiredAt = at };
        _definitions[key] = retired;

        await EmitAsync(tenantId, AuditEventType.TaxonomyVersionRetired, TaxonomyAuditPayloadFactory.VersionRetired(id, version, retiredBy), at, ct).ConfigureAwait(false);
    }
        ...
        CancellationToken ct)
    {
        ...
        await EmitAsync(tenantId, AuditEventType.TaxonomyNodeAdded, TaxonomyAuditPayloadFactory.NodeAdded(node, addedBy), at, ct).ConfigureAwait(false);
        return node;
    }
        ...
        CancellationToken ct)
    {
        ...
        await EmitAsync(tenantId, AuditEventType.TaxonomyNodeDisplayRevised, TaxonomyAuditPayloadFactory.NodeDisplayRevised(nodeId, version, newDisplay, newDescription, revisionReason, revisedBy), at, ct).ConfigureAwait(false);
        return revised;
    }
        ...
        CancellationToken ct)
    {
        ...
        await EmitAsync(tenantId, AuditEventType.TaxonomyNodeTombstoned, TaxonomyAuditPayloadFactory.NodeTombstoned(nodeId, version, deprecationReason, successorCode, tombstonedBy), at, ct).ConfigureAwait(false);
    }
        ...
        CancellationToken ct)
    {
        ...
        var derived = await DeriveAsync(tenantId, source, sourceVersion, newId, TaxonomyLineageOp.Clone, clonedBy, reason, at, ct).ConfigureAwait(false);

        await EmitAsync(tenantId, AuditEventType.TaxonomyDefinitionCloned, TaxonomyAuditPayloadFactory.Cloned(source, sourceVersion, newId, clonedBy, reason), at, ct).ConfigureAwait(false);
        return derived;
    }
        ...
        CancellationToken ct)
    {
        ...
        var derived = await DeriveAsync(tenantId, source, sourceVersion, newId, TaxonomyLineageOp.Extend, extendedBy, reason, at, ct).ConfigureAwait(false);

        await EmitAsync(tenantId, AuditEventType.TaxonomyDefinitionExtended, TaxonomyAuditPayloadFactory.Extended(source, sourceVersion, newId, extendedBy, reason), at, ct).ConfigureAwait(false);
        return derived;
    }
        ...
        CancellationToken ct)
    {
        ...
        var derived = await DeriveAsync(tenantId, source, sourceVersion, newId, TaxonomyLineageOp.Alter, alteredBy, reason, at, ct).ConfigureAwait(false);

        await EmitAsync(tenantId, AuditEventType.TaxonomyDefinitionAltered, TaxonomyAuditPayloadFactory.Altered(source, sourceVersion, newId, alteredBy, reason), at, ct).ConfigureAwait(false);
        return derived;
    }
    ...
    public async Task RegisterCorePackageAsync(TenantId tenantId, TaxonomyCorePackage package, CancellationToken ct)
    {
        ...
        await EmitAsync(tenantId, AuditEventType.TaxonomyDefinitionCreated, TaxonomyAuditPayloadFactory.Created(package.Definition, package.Definition.Owner), at, ct).ConfigureAwait(false);
    }
    ...
    {
        if (governance == TaxonomyGovernanceRegime.Authoritative && actor != ActorId.Harborline)
        {
