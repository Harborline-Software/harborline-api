...
using Harborline.Api.Blocks.FinancialAr.Models.Events;
...
public sealed class InvoicePostingService : IInvoicePostingService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var invoice = await _invoices.GetAsync(CurrentTenantId, invoiceId, authority.At, cancellationToken).ConfigureAwait(false);
        if (invoice is null)
        ...
        var sourceReference = $"invoice:{invoice.Id.Value}";
        var existingIssueEntry = _journalStore is null
            ? null
            : await _journalStore
                .FindBySourceReferenceAsync(CurrentTenantId, sourceReference, cancellationToken)
                .ConfigureAwait(false);
        var entry = new JournalEntry(
            id: existingIssueEntry?.Id ?? JournalEntryId.NewId(),
        ...
        // Build the issued invoice BEFORE posting so it can be staged onto the JE write's transaction
        // (ADR 0135 F3). The fields are pure functions of the inputs already computed above.
        ...
        // Repair only a legacy pre-declaration state: an issue JE already exists while the invoice is
        // still Draft. No new ledger write occurs here, so this is not the removed post-commit fallback.
        if (existingIssueEntry is not null)
        {
            await _invoices.UpsertAsync(CurrentTenantId, issued, authority.At, cancellationToken).ConfigureAwait(false);
            await PublishAsync(
                AccountsReceivableEventNames.InvoiceIssued,
                new InvoiceIssuedPayload(issued.Id, invoiceNumber, issued.CustomerId, total, issued.DueDate, issued.PropertyId, existingIssueEntry.Id),
                $"invoice-issued:{issued.Id.Value}",
                issued.TenantId,
                authority.At,
                cancellationToken).ConfigureAwait(false);
            return new IssueResult(issued, existingIssueEntry.Id, IssueError.None, "Repaired legacy stranded Draft; no journal write performed.");
        }

        // ADR 0135 F3 — the declared journal operation requires the issued-invoice adapter. It stages this
        // update in the journal transaction or refuses the post; there is no post-commit recovery write.
        var pendingIssued = new PendingIssuedInvoiceUpdate(CurrentTenantId, issued, sourceReference);
        PostResult postResult;
        using (IssuedInvoiceWriteScope.Enter(pendingIssued))
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _invoices.UpsertAsync(CurrentTenantId, writtenOff, authority.At, cancellationToken).ConfigureAwait(false);
    ...
    private void RequireTenant(AuthorizationWriteContext authority)
    {
        if (authority.Tenant != CurrentTenantId)
            throw new ArgumentException("The invoice tenant does not match the boundary authority.", nameof(authority));
    }
