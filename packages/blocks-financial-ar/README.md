...
Accounts-receivable substrate for Harborline. Builds on canonical Party (`blocks-people-foundation`) and the GL ledger (`blocks-financial-ledger`).
...
- **PR 3:** `IInvoicePostingService` — issue / void / write-off operations, tax computation via `ITaxCalculationService`, idempotent journal-entry posting + canonical-event emission via `Harborline.Api.Foundation.Events.IDomainEventPublisher`.
...
| Origin | Used here |
|---|---|
| `Harborline.Api.Blocks.FinancialLedger.Models` | `GLAccountId`, `ChartOfAccountsId`, `JournalEntryId` |
| `Harborline.Api.Blocks.People.Foundation.Models` | `PartyId` (the customer FK) |
| `Harborline.Api.Foundation.Assets.Common` | `TenantId`, `Instant` |

`TaxCodeId` is intentionally a `string?` rather than `Harborline.Api.Blocks.FinancialTax.Models.TaxCodeId` — keeping AR independent of the tax cluster's exact ID type until PR 3 wires the calculation engine.

## Build + test

```bash
dotnet build packages/blocks-financial-ar/Harborline.Blocks.FinancialAr.csproj
dotnet test  packages/blocks-financial-ar/tests/Harborline.Blocks.FinancialAr.Tests.csproj
```
