...
/// first-slice tests. The kernel-audit substrate (see <c>Harborline.Api.Kernel.Audit</c>)
/// is the eventual emission target; first-slice carries the domain event in
/// the in-memory event store only (see <c>Harborline.Api.Blocks.PropertyEquipment.Services.IEquipmentLifecycleEventStore</c>).
...
public sealed record EquipmentLifecycleEvent : IMustHaveTenant
{
    ...
    /// shape (added by product management) — see PR description for rationale.
    /// </summary>
