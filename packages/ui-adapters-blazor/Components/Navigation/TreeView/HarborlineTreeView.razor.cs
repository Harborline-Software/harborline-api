...
using Harborline.Api.UIAdapters.Blazor.Internal;
...
public partial class HarborlineTreeView : HarborlineComponentBase
{
    // ── Internal state ──────────────────────────────────────────────────
    ...
    // ── Internal API (used by HarborlineTreeItem) ───────────────────────────
        ...
        foreach (var node in nodes)
        {
            ...
            if (OnItemContextMenu.HasDelegate)
            {
                ...
                builder.AddAttribute(18, "oncontextmenu", EventCallback.Factory.Create<MouseEventArgs>(this, (MouseEventArgs args) =>
                    OnItemContextMenu.InvokeAsync(new TreeItemContextMenuEventArgs { Item = ctxNode.Item, ItemId = ctxNode.Id, MouseEventArgs = args.ToHarborline() })));
            ...
            if (!string.IsNullOrEmpty(node.Icon))
            {
                ...
                builder.AddContent(42, IconProvider.GetIcon(node.Icon, Harborline.Api.Foundation.Enums.IconSize.Small));
                builder.CloseElement();
            }
