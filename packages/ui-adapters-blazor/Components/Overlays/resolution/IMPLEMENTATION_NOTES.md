# Implementation Notes: HarborlineWindow (Overlays)
...
Follows the established project pattern (HarborlineSplitter, HarborlineTooltip, HarborlineMediaQuery):
...
- Portal rendering via a spec-level root component
...
```
Overlays/
├── HarborlineWindow.razor          — Main window component (single file, razor + @code)
...
- `Harborline.Core/Enums/WindowEnums.cs` — Added `WindowAnimationType`, `WindowSize`, `WindowFooterLayoutAlign` enums
- `Harborline.Core/Contracts/IHarborlineCssProvider.cs` — Added `WindowFooterClass()` method
- `Harborline.Providers.FluentUI/FluentUICssProvider.cs` — Implemented `WindowFooterClass()`
- `Harborline.Providers.Bootstrap/BootstrapCssProvider.cs` — Implemented `WindowFooterClass()`

### Child Component Communication
Child components (`WindowContent`, `WindowTitle`, `WindowFooter`, `WindowActions`) receive the parent `HarborlineWindow` via `CascadingValue`. On `OnInitialized`, they call `internal` setters on the parent to register their content fragments. The parent renders these fragments in the appropriate locations.

`WindowActionButton` registers itself with the parent via `RegisterActionButton()` and delegates click handling to `internal` methods on `HarborlineWindow` (`HandleCloseFromAction`, `HandleMinimizeFromAction`, `HandleMaximizeRestoreFromAction`) when no custom `OnClick` is bound.
