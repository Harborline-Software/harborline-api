...
namespace Harborline.Api.UIAdapters.Blazor.Components.Scheduling;

/// <summary>
/// Canonical MVP scheduler for the Harborline Scheduling component family.
...
/// <c>Harborline.Api.UIAdapters.Blazor.Components.Scheduling</c> namespace. A non-generic
/// <c>Harborline.Api.UIAdapters.Blazor.Components.DataDisplay.HarborlineScheduler</c> predates this
...
/// </para>
/// </remarks>
public partial class HarborlineScheduler<TEvent> : HarborlineComponentBase
    where TEvent : class
{
    ...
    /// <summary>
    /// View-local presentation-role mapping. Mapped fields take precedence over the legacy field
    /// parameters below; those parameters remain as compatibility fallbacks for existing callers.
    /// </summary>
    [Parameter] public ShapeRoleMapping? ShapeRoles { get; set; }
    ...
    private string GetTitle(TEvent item) =>
        GetField(item, ShapeRoles?.Title ?? TitleField)?.ToString() ?? string.Empty;

    private object? GetId(TEvent item) => GetField(item, IdField);

    private DateTime GetStart(TEvent item) =>
        ParseDate(GetField(item, ShapeRoles?.PlacedBy ?? StartField)) ?? DateTime.Today;
