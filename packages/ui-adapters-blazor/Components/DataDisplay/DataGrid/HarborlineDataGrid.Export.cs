...
/// <summary>
/// CSV export logic for <see cref="HarborlineDataGrid{TItem}"/>.
/// </summary>
public partial class HarborlineDataGrid<TItem>
{
    ...
    /// <summary>
    /// Lazy-loaded reference to <c>harborline-clipboard-download.js</c>.
    ...
    public async Task ExportToCsvAsync(CsvExportOptions options)
    {
        ...
        var fileName = string.IsNullOrWhiteSpace(options.FileName)
            ? $"grid-export-{_timeProvider.GetUtcNow().UtcDateTime:yyyyMMddHHmmss}.csv"
            : options.FileName;
    ...
    /// <summary>
    /// Triggers a browser file download using the <c>harborline-clipboard-download.js</c> module.
    ...
    private async Task TriggerCsvDownloadAsync(string fileName, string mimeType, string content)
    {
        try
        {
            _clipboardDownloadModule ??= await JS.InvokeAsync<IJSObjectReference>(
                "import",
                HarborlineJsModuleLoader.Resolve("js/harborline-clipboard-download.js"));
    ...
    /// <c>HarborlineDataGrid.Interop.cs</c>.
    /// </summary>
