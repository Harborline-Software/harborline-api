...
/// <summary>
/// PDF export logic for <see cref="HarborlineDataGrid{TItem}"/>.
/// </summary>
public partial class HarborlineDataGrid<TItem>
{
    // Resolved lazily so rendering succeeds without a registered IPdfExportWriter.
    ...
    /// <c>harborline-clipboard-download.js</c>). This requires interactive rendering (WebAssembly
    ...
    /// </para>
    /// <para>
    /// <b>PDF service:</b> PDF export requires an <see cref="IPdfExportWriter"/> registered in
    /// your application's DI container. No adapter for this grid-local writer ships in this
    /// repository; the host supplies one.
    /// </para>
    /// </remarks>
    ...
    public async Task ExportToPdfAsync(PdfExportOptions? options = null)
    {
        ...
        var pdfWriter = PdfWriter
            ?? throw new InvalidOperationException(
                "PDF export requires an IPdfExportWriter registration in your DI container.");
        ...
        var fileName = string.IsNullOrWhiteSpace(options.FileName)
            ? $"grid-export-{_timeProvider.GetUtcNow().UtcDateTime:yyyyMMddHHmmss}.pdf"
            : options.FileName;
