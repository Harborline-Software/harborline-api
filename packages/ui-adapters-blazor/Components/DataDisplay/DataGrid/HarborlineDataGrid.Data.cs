using Harborline.Api.Foundation.Data;
...
/// <summary>
/// Data processing pipeline for HarborlineDataGrid: filtering, sorting, grouping, paging.
/// </summary>
public partial class HarborlineDataGrid<TItem>
{
    // ── Data Processing ─────────────────────────────────────────────────
    ...
    public async Task ExportToExcelAsync(XlsxExportOptions? options = null)
    {
        ...
        // Generate XLSX bytes through the optional server/export companion.
        var xlsxWriter = _exportServices.GetService<IXlsxExportWriter>()
            ?? throw new InvalidOperationException(
                "XLSX export requires Harborline.Api.UIAdapters.Blazor.Server and its " +
                "XlsxExportWriter registration.");
        var bytes = xlsxWriter.Write(columns, itemList, options);
    ...
    private async Task TriggerXlsxDownloadAsync(byte[] bytes, XlsxExportOptions options)
    {
        var fileName = string.IsNullOrWhiteSpace(options.FileName)
            ? $"grid-export-{_timeProvider.GetUtcNow().UtcDateTime:yyyyMMddHHmmss}.xlsx"
            : options.FileName;
        ...
        // function in harborline-clipboard-download.js.  Base64 encoding is the standard
    ...
    internal async Task OnHeaderClick(HarborlineGridColumn<TItem> column, bool isSortable, MouseEventArgs? e = null)
    {
    ...
    internal bool IsSortedColumn(HarborlineGridColumn<TItem> column)
    ...
    internal async Task SortColumnAsc(HarborlineGridColumn<TItem> column)
    {
        if (!Sortable || !column.Sortable) return;
        var existing = _state.SortDescriptors.FirstOrDefault(s => s.Field == column.Field);
        if (existing is not null)
            existing.Direction = Harborline.Api.Foundation.Enums.SortDirection.Ascending;
        else
        {
            _state.SortDescriptors.Clear();
            _state.SortDescriptors.Add(new Harborline.Api.Foundation.Data.SortDescriptor
            {
                Field = column.Field,
                Direction = Harborline.Api.Foundation.Enums.SortDirection.Ascending
            });
        }
        _state.CurrentPage = 1;
        await ProcessDataAsync();
    ...
    internal async Task SortColumnDesc(HarborlineGridColumn<TItem> column)
    {
        if (!Sortable || !column.Sortable) return;
        var existing = _state.SortDescriptors.FirstOrDefault(s => s.Field == column.Field);
        if (existing is not null)
            existing.Direction = Harborline.Api.Foundation.Enums.SortDirection.Descending;
        else
        {
            _state.SortDescriptors.Clear();
            _state.SortDescriptors.Add(new Harborline.Api.Foundation.Data.SortDescriptor
            {
                Field = column.Field,
                Direction = Harborline.Api.Foundation.Enums.SortDirection.Descending
            });
        }
        _state.CurrentPage = 1;
        await ProcessDataAsync();
    ...
    internal async Task ClearColumnSort(HarborlineGridColumn<TItem> column)
    {
        var existing = _state.SortDescriptors.FirstOrDefault(s => s.Field == column.Field);
        if (existing is null) return;
        _state.SortDescriptors.Remove(existing);
    ...
    /// </summary>
    internal void OpenFilterMenu(HarborlineGridColumn<TItem> column)
    {
    ...
    /// <summary>
    /// Toggles the <see cref="Harborline.Api.UIAdapters.Blazor.Components.DataDisplay.HarborlineGridColumn{TItem}.Locked"/>
    ...
    /// </summary>
    internal async Task ToggleColumnLock(HarborlineGridColumn<TItem> column)
    {
