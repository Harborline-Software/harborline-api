...
- Column width is stored as `EffectiveWidth` on each `HarborlineGridColumn<TItem>` component instance[cite:44].
...
3. **Renderers are consumers only.** `HarborlineDataGrid.Rendering.cs` and `HarborlineDataGrid.razor` must not perform independent width calculations. They receive a layout contract and apply it[cite:18][cite:23].
...
```csharp
namespace Harborline.Components.DataGrid.Sizing;

/// <summary>
/// Computes the authoritative column widths for a HarborlineDataGrid layout pass.
...
public interface IColumnWidthProvider
{
    ...
    /// </summary>
    /// <param name="columns">
    /// Ordered list of column definitions as registered by HarborlineGridColumn components.
...
/// <summary>
/// A sizing snapshot of one column, derived from HarborlineGridColumn parameters.
...
/// This is the Phase 1 default for HarborlineDataGrid.
/// </summary>
...
/// This is the Phase 2 default for HarborlineDataGrid.
/// </summary>
...
/// Use a ResizeObserver on the grid root element (HarborlineDataGrid.Interop.cs)
...
public sealed class MeasuredAutoWidthProvider : IColumnWidthProvider
{
    ...
    /// <summary>
    /// Called by HarborlineDataGrid.Interop.cs after the JS measurement pass completes.
...
## Integration with HarborlineDataGrid

### Step 1 — Add `IColumnWidthProvider` to `HarborlineDataGrid.razor.cs`
...
In `HarborlineDataGrid.razor`, the header row and each body row get the same CSS custom property:
...
In `HarborlineDataGrid.Interop.cs`, add a `ResizeObserver` callback that updates `_containerWidth` and calls `ResolveLayout()`:
...
| Add `_widthProvider`, `_layoutContract`, `_containerWidth` to grid state | `HarborlineDataGrid.razor.cs` |
| Implement `BuildSizingEntries()` and `ResolveLayout()` | `HarborlineDataGrid.razor.cs` |
| Refactor header rendering to use `_layoutContract` | `HarborlineDataGrid.Rendering.cs` |
| Refactor body row rendering to use `_layoutContract` | `HarborlineDataGrid.razor` |
...
| Add `Flex` parameter to `HarborlineGridColumn.razor` | `HarborlineGridColumn.razor` |
| Connect `ResizeObserver` JS interop to `_containerWidth` | `HarborlineDataGrid.Interop.cs` |
| Auto-select `FlexWidthProvider` when any column has `Flex` set | `HarborlineDataGrid.razor.cs` |
...
| Task | File(s) affected |
|---|---|
| Implement header cell resize handle (`marilo-grid-resize-handle`) | `HarborlineDataGrid.razor` |
| On pointer-move: call `_widthProvider.ApplyResize(_layoutContract, colId, delta)` | `HarborlineDataGrid.Rendering.cs` |
| On pointer-up: commit final contract, notify `OnStateChanged` | `HarborlineDataGrid.razor.cs` |
| Expose `Resizable` on `HarborlineGridColumn.razor` | `HarborlineGridColumn.razor` |
...
| Add `AutoWidth` parameter to `HarborlineGridColumn.razor` | `HarborlineGridColumn.razor` |
| Call `CommitMeasuredWidths()` after first render and after column definition changes | `HarborlineDataGrid.Interop.cs` |
| Guard against zero-width container measurements | `HarborlineDataGrid.Interop.cs` |
...
These patterns are explicitly disallowed in `HarborlineDataGrid` rendering code after Phase 1 is merged. Any PR that introduces them must be rejected:
...
```
src/Harborline.Components/DataGrid/
├── Sizing/
...
├── HarborlineDataGrid.razor
├── HarborlineDataGrid.razor.cs
├── HarborlineDataGrid.Rendering.cs
├── HarborlineDataGrid.Interop.cs
├── HarborlineDataGrid.Data.cs
├── HarborlineDataGrid.Editing.cs
├── HarborlineGridColumn.razor
├── GridState.cs
├── GridEventArgs.cs
└── GridCommandTypes.cs
```
