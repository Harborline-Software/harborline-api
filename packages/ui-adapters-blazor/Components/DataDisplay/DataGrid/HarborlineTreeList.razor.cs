...
public partial class HarborlineTreeList<TItem> : HarborlineComponentBase, IColumnHost, ITreeListEditController
{
...
#pragma warning disable CS0618
    [Parameter][Obsolete("Use <HarborlineTreeListColumn> child components instead.")]
...
#pragma warning restore CS0618
    ...
    private readonly List<HarborlineColumnBase> _registeredColumns = new();
    ...
    void IColumnHost.RegisterColumn(HarborlineColumnBase column)
    {
        if (!_registeredColumns.Contains(column)) { _registeredColumns.Add(column); InvokeAsync(StateHasChanged); }
    }

    void IColumnHost.UnregisterColumn(HarborlineColumnBase column)
    {
...
#pragma warning restore CS0618
    ...
    internal int ExpandableColumnIndex
    {
        get
        {
            ...
            {
                if (cols[i] is HarborlineTreeListColumn { Expandable: true }) return i;
            }
            return 0;
        }
    }
    ...
    {
        if (col is HarborlineColumnBase mcb && mcb.Sortable.HasValue) return mcb.Sortable.Value;
        return Sortable;
    }
    ...
    {
        if (col is HarborlineColumnBase mcb && mcb.Filterable.HasValue) return mcb.Filterable.Value;
        return true;
    }
    ...
    private RenderFragment RenderRows(List<TreeListNode> nodes, int depth) => builder =>
    {
        ...
        foreach (var node in nodes)
        {
            ...
            {
                var col = columns[ci]; var cellTemplate = (col as HarborlineTreeListColumn)?.Template;
                ...
                if (isEditingRow)
                {
                    ...
                    var editTpl = (col as HarborlineTreeListColumn)?.EditTemplate;
                    if (ci == expandCol)
                    {
