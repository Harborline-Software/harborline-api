...
public partial class HarborlineDataGrid<TItem> : HarborlineComponentBase
{
    // ── Internal state ─────────────────────────────────────────────────
    internal readonly List<HarborlineGridColumn<TItem>> _columns = [];
    ...
    // Editing state (managed in HarborlineDataGrid.Editing.cs)
    ...
    private readonly Dictionary<HarborlineGridColumn<TItem>, string> _columnSizingIds = [];
    ...
    /// disturbing explicit <see cref="HarborlineGridColumn{TItem}"/> children.</summary>
    internal readonly List<HarborlineGridColumn<TItem>> _autoGeneratedColumns = [];

    // ── DI services ─────────────────────────────────────────────────────
    [Inject] private IDownloadService _downloadService { get; set; } = default!;
    [Inject] private IServiceProvider _exportServices { get; set; } = default!;
    [Inject] private TimeProvider _timeProvider { get; set; } = default!;
    ...
    /// <summary>Column definitions (HarborlineGridColumn components).</summary>
    ...
    internal void RegisterColumn(HarborlineGridColumn<TItem> column)
    {
    ...
    internal void UnregisterColumn(HarborlineGridColumn<TItem> column)
    {
    ...
    internal List<HarborlineGridColumn<TItem>> _visibleColumns => _columns.Where(c => c.Visible).ToList();
    ...
    protected override async Task OnParametersSetAsync()
    {
        ...
        // Explicit <HarborlineGridColumn> children take priority; auto-generated columns fill the gaps.
    ...
    private string? GetSizingId(HarborlineGridColumn<TItem> column)
        => _columnSizingIds.TryGetValue(column, out var id) ? id : null;

    internal string? GetResolvedColumnWidth(HarborlineGridColumn<TItem> column)
    {
    ...
    internal string? GetColumnWidthStyle(HarborlineGridColumn<TItem> column)
    {
        var width = GetResolvedColumnWidth(column);
        return width is null ? null : $"width:{width};";
    }

    internal string? GetColumnCellStyle(HarborlineGridColumn<TItem> column, string? extraStyle = null)
    {
    ...
    internal string? GetFrozenCellStyle(HarborlineGridColumn<TItem> column, bool isHeader = false)
    {
    ...
    /// </summary>
    internal bool IsBoundaryFrozenColumn(HarborlineGridColumn<TItem> column)
    {
    ...
    /// Explicit <see cref="HarborlineGridColumn{TItem}"/> children (those NOT in <see cref="_autoGeneratedColumns"/>)
    ...
    private void RefreshAutoGeneratedColumns()
    {
        ...
        {
            var col = new HarborlineGridColumn<TItem>();
            typeof(HarborlineGridColumn<TItem>).GetProperty(nameof(HarborlineGridColumn<TItem>.Field))!.SetValue(col, prop.Name);
            typeof(HarborlineGridColumn<TItem>).GetProperty(nameof(HarborlineGridColumn<TItem>.Title))!.SetValue(col, title);
            if (!editable)
                typeof(HarborlineGridColumn<TItem>).GetProperty(nameof(HarborlineGridColumn<TItem>.Editable))!.SetValue(col, false);
