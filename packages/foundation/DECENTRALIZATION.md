# Harborline Foundation — Decentralization Primitives

Four namespaces in `Harborline.Api.Foundation` implement Harborline's decentralization primitives. All ship in the single `Harborline.Api.Foundation` assembly. **No Blazor or AspNetCore dependency** — the `HasNoBlazorDependency` invariant is test-locked.

| Namespace | Role | Spec |
|---|---|---|
| `Harborline.Api.Foundation.Crypto` | Ed25519 signer/verifier, `SignedOperation<T>` envelope, canonical JSON | §2.4, §10.2.1 |
| `Harborline.Api.Foundation.Capabilities` | Keyhive-inspired capability graph (principals, groups, delegates) | §10.2.1 |
| `Harborline.Api.Foundation.Macaroons` | HMAC-SHA256 bearer tokens, first-party caveats, attenuation | §10.2.2 |
| `Harborline.Api.Foundation.PolicyEvaluator` | OpenFGA-style ReBAC over the capability graph | §3.5 |

See the platform specification (a pre-Harborline document, not in this repository) and `docs/specifications/research-notes/automerge-evaluation.md` (Keyhive) + `external-references.md` §3.1 (OpenFGA) for the research basis.

## What the primitives compose

Harborline's decentralization story has three interlocking parts:
...
```csharp
services.AddHarborline()
        .AddHarborlineDecentralization(o =>
        {
...
- **Automerge CRDT library integration** — Harborline adopts the Keyhive **model**, not the library.
