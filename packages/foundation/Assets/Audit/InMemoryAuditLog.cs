...
public sealed class InMemoryAuditLog : IAuditLog
{
    ...
    public Task<AuditId> AppendAsync(AuditAppend append, CancellationToken ct = default) =>
        _storage.ExecuteExclusiveAsync(() => AppendCoreAsync(append), ct);

    private Task<AuditId> AppendCoreAsync(AuditAppend append)
    {
    ...
    public async IAsyncEnumerable<AuditRecord> QueryAsync(AuditQuery query, [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        var source = await _storage.ExecuteExclusiveAsync(() => Task.FromResult(
            query.Entity is { } entity
                ? _storage.Audit.TryGetValue(entity, out var chain)
                    ? chain.ToArray()
                    : Array.Empty<AuditRecord>()
                : _storage.Audit.Values.SelectMany(static c => c.ToArray()).ToArray()), ct).ConfigureAwait(false);
    ...
    public Task<bool> VerifyChainAsync(EntityId entity, CancellationToken ct = default) =>
        _storage.ExecuteExclusiveAsync(() => Task.FromResult(VerifyChain(entity)), ct);

    private bool VerifyChain(EntityId entity)
    {
        if (!_storage.Audit.TryGetValue(entity, out var chain))
            return true;
        var snapshot = chain.OrderBy(r => r.At).ThenBy(r => r.Id.Value).ToList();
        return HashChain.Verify(snapshot);
    }
