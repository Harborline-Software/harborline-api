    ...
    IGrantStore grants)
{
    ...
    private static readonly IReadOnlyDictionary<string, RoleBindingSet> ReviewedOffers =
        new Dictionary<string, RoleBindingSet>(StringComparer.Ordinal)
        {
            ...
            [Permission.StoriesRead] = Roles(member: true),
            [Permission.StoriesCreate] = Roles(member: true),
            [Permission.StoriesWrite] = Roles(member: true),
            [Permission.StoriesArchive] = Roles(member: true),
