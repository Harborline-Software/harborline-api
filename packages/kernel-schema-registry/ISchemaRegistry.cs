...
/// in the platform gap analysis of 2026-04-18 (a pre-Harborline
/// discovery artifact, no longer in the repository set).
/// The namespace <c>Harborline.Api.Kernel.Schema</c> is preserved from the G1 reserved
/// stub; the assembly that owns the type moved from <c>Harborline.Api.Kernel</c> to
/// <c>Harborline.Api.Kernel.SchemaRegistry</c> when the stub was promoted.
/// </para>
/// <para>
...
public interface ISchemaRegistry
{
    ...
    /// <summary>
    /// Parses, canonicalizes, content-addresses, and registers the
...
/// human-readable English description produced by the underlying validator.
/// </summary>
/// <param name="JsonPointer">RFC 6901 pointer into the failing document node (empty string = document root).</param>
/// <param name="Message">Human-readable English description (the engine/UI fallback when no localized template exists).</param>
/// <param name="Code">
/// Stable, locale-independent failure code — the failing JSON-Schema keyword
/// (<c>required</c> / <c>type</c> / <c>minimum</c> / <c>maximum</c> / <c>enum</c> /
/// <c>minLength</c> / <c>maxLength</c> / <c>pattern</c> / …). A localization client keys
/// a message template off this code instead of parsing <see cref="Message"/>. Null only
/// for a synthetic aggregate error with no single owning keyword.
/// </param>
/// <param name="Params">
/// The keyword's structured constraint values, as a string-keyed map — e.g.
/// <c>{ "min": "1" }</c> for <c>minimum</c>, <c>{ "max": "8" }</c> for <c>maxLength</c>,
/// <c>{ "allowed": "[\"a\",\"b\"]" }</c> for <c>enum</c>, <c>{ "field": "assetId" }</c> for a
/// split <c>required</c> error. Lets a localization client interpolate the constraint into
/// its translated template. Null when the keyword carries no extractable constraint value.
/// </param>
