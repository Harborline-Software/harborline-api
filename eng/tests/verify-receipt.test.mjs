...
test('a lane requires exactly its own steps, and the two lanes together are the whole gate', async () => {
  ...
  assert.deepEqual(stepIdsForLane('host'), hostStepIds)
  // No step may fall between the two lanes: a gap is a step CI stops running without telling anyone.
  assert.deepEqual([...stepIdsForLane('shared'), ...stepIdsForLane('host')].sort(), [...requiredStepIds].sort())
