...
( cd "$land_dir" && dotnet restore apps/local-node-host/tests/tests.csproj >/dev/null 2>&1 && ( for d in apps/capability-host; do [ -d "$d" ] && ( cd "$d" && npm ci --silent --no-audit --no-fund >/dev/null 2>&1 ) || true; done ) && bash eng/verify.sh ) || { preserve_land_evidence "$root" "$land_dir" "$head_sha" || echo "land: WARNING could not preserve exact-clone evidence" >&2; echo "land: gate RED on the merge commit; nothing landed"; exit 1; }
...
gate_main='verify_dir="$root/.claude/worktrees/land-verify-$$"; git worktree add --detach "$verify_dir" origin/main -q && ( cd "$verify_dir" && dotnet restore apps/local-node-host/tests/tests.csproj >/dev/null 2>&1 && bash eng/verify.sh ); rc=$?; git worktree remove --force "$verify_dir" >/dev/null 2>&1 || true; [ $rc -eq 0 ]'
