...
#   3. build the platform feed there (the api restores Harborline.Contracts from it since 314 s2), then run bash eng/verify.sh (all 11 steps)
...
( cd "$land_dir" && node eng/build-local-feed.mjs >/dev/null 2>&1 && dotnet restore apps/local-node-host/tests/tests.csproj >/dev/null 2>&1 && ( for d in apps/capability-host; do [ -d "$d" ] && ( cd "$d" && npm ci --silent --no-audit --no-fund >/dev/null 2>&1 ) || true; done ) && HARBORLINE_GATE_QUALITY=1 bash eng/verify.sh ) && ( cd "$land_dir" && node eng/verify-receipt.mjs --landing ) || { preserve_land_evidence "$root" "$land_dir" "$head_sha" || echo "land: WARNING could not preserve exact-clone evidence" >&2; echo "land: gate RED on the merge commit; nothing landed"; exit 1; }
