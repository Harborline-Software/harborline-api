...
public sealed class T433AccessReplacementProducerTests
{
    ...
    public void Initial_pack_declares_exact_journey_definitions_and_actions()
    {
        ...
        Assert.Equal("access.holder.read", holders.GetProperty("authorizationCapability").GetString());
        Assert.Equal(new[]
        {
    ...
    public void Replacement_artifact_is_hash_pinned_decodable_and_signature_verified()
    {
        ...
        Assert.Equal("cccd647381ca9be4e474babd9ef277edd90269833fc3684c62a40de8c06d7615", hash);

        var codec = new PackFileCodec();
        var file = Assert.IsType<PackFile>(codec.TryDecode(first));
        var envelope = file.Envelope;
        Assert.NotNull(envelope);
        Assert.Equal("ebVWLo_mVPlAeLES6KmLp5AfhTrmlb7X4OORC60ElmQ", envelope.IssuerId.ToBase64Url());
    ...
    public void Replacement_manifest_pins_exact_workflow_and_bound_view_change_sets()
    {
        ...
        Assert.Equal("cccd647381ca9be4e474babd9ef277edd90269833fc3684c62a40de8c06d7615", root.GetProperty("sha256").GetString());
