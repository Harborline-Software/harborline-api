...
{
    [Fact]
    public async Task Queued_activation_precedes_later_readers_without_blocking_nested_reads()
    {
        var order = new System.Collections.Concurrent.ConcurrentQueue<string>();
        var holder = PackProjectionActivationBarrier.Read();
        using var deadline = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        var writer = StartIsolated(() =>
        {
            using var transaction = new PackProjectionTransaction(deadline.Token);
            order.Enqueue("activation");
            transaction.Commit();
        });
        (Thread Thread, Task Completion)? reader = null;
        try
        {
            AssertWaiting(writer);
            reader = StartIsolated(() =>
            {
                using var lease = PackProjectionActivationBarrier.Read(deadline.Token);
                order.Enqueue("read");
            });
            AssertWaiting(reader.Value);
            using var nested = PackProjectionActivationBarrier.Read(deadline.Token);
        }
        finally
        {
            holder.Dispose();
            await writer.Completion;
            if (reader is { } pending) await pending.Completion;
        }
        Assert.Equal(["activation", "read"], order.ToArray());
    }

    [Fact]
    public async Task Cancelled_queued_activation_releases_later_readers_while_original_reader_remains()
    {
        using var holder = PackProjectionActivationBarrier.Read();
        ...
        try
        {
            ...
            await reader.Value.Completion.WaitAsync(TimeSpan.FromSeconds(5));
        }
        finally
        {
            cancellation.Cancel();
            holder.Dispose();
            await writer.Completion.ConfigureAwait(
                ConfigureAwaitOptions.SuppressThrowing | ConfigureAwaitOptions.ContinueOnCapturedContext);
            if (reader is { } pending) await pending.Completion;
        }
    }

    private static (Thread Thread, Task Completion) StartIsolated(Action operation)
    {
    ...
    private static void AssertWaiting((Thread Thread, Task Completion) operation)
    {
        // The operation has no blocking work except Enter's Monitor.Wait. Observing that wait
        // establishes queue admission before the next contender starts; no sleep chooses ordering.
        Assert.True(SpinWait.SpinUntil(() => operation.Completion.IsCompleted
            || (operation.Thread.ThreadState & ThreadState.WaitSleepJoin) != 0, TimeSpan.FromSeconds(5)));
