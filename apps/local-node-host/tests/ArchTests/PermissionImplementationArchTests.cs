...
{
    [Fact(DisplayName = "Every permission atom has an executable consumer outside catalog and seed")]
    public void EveryAtom_HasAnImplementation()
    {
        ...
        var orphans = atoms.Keys.Except(implemented, StringComparer.Ordinal).Order(StringComparer.Ordinal).ToArray();
        Assert.True(orphans.Length == 0, "Permission atoms without implementing readers: "
