...
public sealed class RawMutationPortSymbolInventoryTests
{
        ...
        Func<Type, bool>? typeFilter = null)
    {
        ...
        foreach (var assembly in assemblies.Distinct())
        {
            ...
            {
                var callerSymbol = NormalizeClosureOrdinals(MethodSignatureSymbol.ForCaller(caller));
    ...
    /// <summary>
    /// A closure's display-class ORDINAL is a compiler artefact, not a fact about the call site: it shifts
    /// when an unrelated lambda or constructor parameter appears above it, and it can differ between the
    /// Windows and macOS compilers (ticket 151 slice 2 round 2 — the platform-dependent inventory red). The
    /// inventory pins WHICH method calls a raw mutation port, so the ordinal is normalized away.
    /// </summary>
    private static string NormalizeClosureOrdinals(string symbol) =>
        ClosureOrdinal.Replace(symbol, "<>c__DisplayClass_");

    private static readonly System.Text.RegularExpressions.Regex ClosureOrdinal =
        new(@"<>c__DisplayClass\d+_", System.Text.RegularExpressions.RegexOptions.Compiled);
