...
/// <summary>
/// HTTP-contract coverage for the read-only report definition route family (ticket 085), mapped
...
/// </summary>
public sealed class ReportDefinitionRouteTests : IAsyncLifetime
{
    ...
    private InMemoryReportDefinitionRegistry _registry = null!;
    ...
    public async Task InitializeAsync()
    {
        ...
        _registry = new InMemoryReportDefinitionRegistry(new AcceptAllDescriptorRegistry());
        ...
        ReportDefinitionRoutes.Map(_app.MapDesktopPlaneOnlyGroup(), _registry, _activeTeam);
    ...
    {
        var rows = await _client.GetFromJsonAsync<JsonElement>(ReportDefinitionRoutes.RouteBase);
    ...
    {
        var detail = await _client.GetFromJsonAsync<JsonElement>($"{ReportDefinitionRoutes.RouteBase}/trial");

        Assert.Equal("1.10.0", detail.GetProperty("version").GetString());
        Assert.Equal("standard", detail.GetProperty("reportKind").GetString());
        Assert.Equal(1, detail.GetProperty("schemaVersion").GetInt32());
        Assert.Equal(JsonValueKind.Object, detail.GetProperty("parameters").ValueKind);
        ...
        Assert.Equal(HttpStatusCode.NotFound,
            (await _client.GetAsync($"{ReportDefinitionRoutes.RouteBase}/foreign")).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound,
            (await _client.GetAsync($"{ReportDefinitionRoutes.RouteBase}/unknown")).StatusCode);
    }
    ...
    public async Task Every_404_carries_a_machine_code_and_no_English()
    {
        ...
        var unknownKey = await ReadBodyAsync($"{ReportDefinitionRoutes.RouteBase}/unknown");
        var unknownVersions = await ReadBodyAsync($"{ReportDefinitionRoutes.RouteBase}/unknown/versions");
        var unknownVersion = await ReadBodyAsync($"{ReportDefinitionRoutes.RouteBase}/trial/versions/0.0.0-absent");

        Assert.Equal("report_definition.not_found", unknownKey.GetProperty("code").GetString());
        Assert.Equal("report_definition.not_found", unknownVersions.GetProperty("code").GetString());
        Assert.Equal("report_definition.version_not_found", unknownVersion.GetProperty("code").GetString());
    ...
    public async Task Versions_lists_newest_first_with_the_ordering_rule_named()
    {
        var history = await _client.GetFromJsonAsync<JsonElement>(
            $"{ReportDefinitionRoutes.RouteBase}/trial/versions");
    ...
    public async Task One_version_is_an_exact_pinned_read_with_404_on_absent_tuples()
    {
        var revision = await _client.GetFromJsonAsync<JsonElement>(
            $"{ReportDefinitionRoutes.RouteBase}/trial/versions/1.9.0");

        Assert.Equal("1.9.0", revision.GetProperty("version").GetString());
        Assert.Equal(HttpStatusCode.NotFound,
            (await _client.GetAsync($"{ReportDefinitionRoutes.RouteBase}/trial/versions/0.0.1")).StatusCode);
    }
    ...
    {
        using var request = new HttpRequestMessage(HttpMethod.Get, ReportDefinitionRoutes.RouteBase);
    ...
    private static ReportDefinition Definition(string tenant, string key, string version, string title)
    {
