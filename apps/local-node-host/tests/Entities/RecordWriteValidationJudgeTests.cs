...
public sealed class RecordWriteValidationJudgeTests
{
    ...
    /// <summary>Pads a legal name past the registered schema's maxLength (512) for the route row.</summary>
    private static readonly string LongTail = new('x', 600);
    ...
    public async Task Legibility_RouteProblemShape()
    {
        ...
        await using var route = await RouteHarness.CreateAsync();

        // ADAPTED (candidate C): the record-write route this slice ships is the legal-entity POST, whose
        // typed DTO drops a property the registered schema does not declare — so the refused body is a
        // legal-entity body and the pointer the engine names is /legalName, not /title. The row's property
        // is unchanged: the 422 carries the dotted code and the failing pointer and echoes no value.
        using var response = await route.PostRecordAsync($$"""
            {"legalName":"{{BodyMarker}}{{LongTail}}","kind":"Llc","taxClassification":"DisregardedEntity"}
            """);

        Assert.Equal(HttpStatusCode.UnprocessableEntity, response.StatusCode);
        var payload = await response.Content.ReadAsStringAsync();
        Assert.Contains("entity.validation", payload);
        Assert.Contains("/legalName", payload);
        Assert.DoesNotContain(BodyMarker, payload);
        Assert.DoesNotContain("StackTrace", payload);
        // The typed DTO, not an anonymous shape, and no prose `error` key.
        Assert.Contains("\"code\"", payload);
        Assert.DoesNotContain("\"error\"", payload);
    }
    ...
    public async Task Legibility_OperatorCliRendersTheRefusal()
    {
        ...
        // ADAPTED: in cli-coverage.json the ROUTE is the key and the verb name is the value, so the
        // route is matched on the property name. The property the row asserts is unchanged.
        var driven = verbs.EnumerateObject()
            .Any(verb => verb.Name.Contains("/api/local-node/entities", StringComparison.Ordinal));
        Assert.True(driven,
        ...
        // ADAPTED (two judge-side fixes): the CLI derives its target from --url / HARBORLINE_NODE_URL, not
        // from the client's BaseAddress, so the URL is passed; and the row is driven through the RECORD-WRITE
        // verb this round adds rather than `health`, which is what the row is actually about.
        var bodyFile = Path.Combine(Path.GetTempPath(), "judge151-entity-" + Guid.NewGuid().ToString("N") + ".json");
        await File.WriteAllTextAsync(bodyFile, """{"legalName":"","kind":"Llc","taxClassification":"DisregardedEntity"}""");
        try
        {
            await Harborline.Api.NodeOperatorCli.OperatorCli.RunAsync(
                ["entity", "create", "--file", bodyFile, "--url", "http://127.0.0.1:9/", "--json"],
    ...
    private sealed class RouteHarness : IAsyncDisposable
    {
        ...
        /// <summary>
        /// The EF-backed harness row 4b needs, built the way <c>EntityRouteTests</c> builds it: a migrated
        /// on-disk SQLite store, the active-team seam, and the PRODUCTION <c>EntityRoutes.Map</c> over the
        /// candidate's own writer and the shipped schema-registry validator. No null object, no accept-all
        /// double.
        /// </summary>
        internal static async Task<RouteHarness> CreateAsync()
        {
            var directory = Path.Combine(
                Path.GetTempPath(), "harborline-judge151-route-" + Guid.NewGuid().ToString("N"));
            ...
            builder.Services.AddDbContextFactory<LocalNodeDbContext>(options => options.UseSqlite(
                $"Data Source={Path.Combine(directory, "judge-151.db")};Pooling=False"));
            var activeTeam = new FixedActiveTeam();
            builder.Services.AddSingleton<Harborline.Api.Kernel.Runtime.Teams.IActiveTeamAccessor>(activeTeam);
            builder.Services.AddScoped<Harborline.Api.Foundation.MultiTenancy.ITenantContext,
                Harborline.Api.LocalNodeHost.Data.Financial.ActiveTeamTenantContext>();
            ...
            var registry = Harborline.Api.LocalNodeHost.Data.Entities.TestNodeRecordSchemas.NewRegistry();
            // The legal-entity POST writes through EF, not the mutation port, so the writer is built on
            // the ctor EntityRouteTests uses — the same shipped validator over the same registry.
            var writer = Harborline.Api.LocalNodeHost.Data.Entities.TestEntityWriters.OverEf(
                factory,
                new EntityBodyAdmission(new Harborline.Api.Kernel.Schema.SchemaRegistryEntityValidator(registry)),
                Harborline.Api.LocalNodeHost.Data.Entities.TestNodeRecordSchemas.Over(registry),
            ...
            Harborline.Api.LocalNodeHost.Health.EntityRoutes.Map(
                app, factory, activeTeam, writer, TimeProvider.System);
        ...
        /// <summary>Posts a record body to the node's record-write route.</summary>
    ...
    /// <summary>One fixed active team — the route's tenant seam, as EntityRouteTests supplies it.</summary>
    private sealed class FixedActiveTeam : Harborline.Api.Kernel.Runtime.Teams.IActiveTeamAccessor
    {
        public Harborline.Api.Kernel.Runtime.Teams.TeamContext? Active { get; } = new(
            new Harborline.Api.Kernel.Runtime.Teams.TeamId(
                Guid.Parse("7e570000-0000-0000-0000-0000000000cc")),
            "Judge 151",
            new ServiceCollection().BuildServiceProvider(),
            TimeProvider.System);

        public Task SetActiveAsync(Harborline.Api.Kernel.Runtime.Teams.TeamId teamId, CancellationToken ct) =>
            throw new NotSupportedException("The judge route harness serves one fixed team.");
