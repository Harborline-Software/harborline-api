...
public sealed class RecordWriteValidationJudgeTests
{
    ...
    {
        // ADAPTATION (candidate A): the row RUNS rather than being skipped — RouteHarness now builds the
        // EF-backed host EntityRouteTests builds (mapped production route, migrated SQLite, active-team
        // seam), so the refusal shape is scored by running. HARBORLINE_151_ROUTE=0 opts back out.
        Skip.If(string.Equals(Environment.GetEnvironmentVariable("HARBORLINE_151_ROUTE"), "0",
            StringComparison.Ordinal), "HARBORLINE_151_ROUTE=0 disables the route row.");
        await using var harness = await Harness.CreateAsync();
        await using var route = await RouteHarness.CreateAsync(harness);

        // The record route the slice validates is the legal-entity POST, so the body and the expected
        // pointer are that schema's: the marker rides in legalName (a value the refusal must NOT echo) and
        // the refusal points at the member the engine names.
        using var response = await route.PostRecordAsync(
            $$"""{"legalName":"{{BodyMarker}}","kind":"NotAKind","taxClassification":"DisregardedEntity"}""");
    ...
    public async Task Legibility_OperatorCliRendersTheRefusal()
    {
        ...
        // ADAPTATION (candidate A): cli-coverage.json maps "METHOD route" -> verb name, so the route is the
        // property NAME and the verb name is the value. Both halves are scanned; only a verbs entry naming
        // the record route satisfies the row (CliVerbRouteBindingTests proves the verb really drives it).
        var driven = verbs.EnumerateObject()
            .Any(verb => verb.Name.Contains("/api/local-node/entities", StringComparison.Ordinal)
                || verb.Value.ToString().Contains("/api/local-node/entities", StringComparison.Ordinal));
        Assert.True(driven,
        ...
        await Harborline.Api.NodeOperatorCli.OperatorCli.RunAsync(
            // ADAPTATION (candidate A): the CLI resolves its node URL from --url or HARBORLINE_NODE_URL,
            // never from the HttpClient's BaseAddress, so the argv carries the (stubbed) listener.
            ["--url", "http://127.0.0.1:9", "--json", "health"], client, stdout, stderr);
    ...
    /// <summary>
    /// The production entity route, mapped as <c>HostedEntityApiEndpoint</c> maps it, over a migrated temp
    /// SQLite store — the recipe <c>EntityRouteTests</c> uses, so row 4b scores the shipped route handler
    /// and not a stand-in (candidate A adaptation: the judge file left this row skipped).
    /// </summary>
    private sealed class RouteHarness : IAsyncDisposable
    {
        ...
        private RouteHarness(
            Microsoft.AspNetCore.Builder.WebApplication app, HttpClient client, string directory)
        {
            _app = app;
            _client = client;
        ...
        internal static async Task<RouteHarness> CreateAsync(Harness harness)
        {
            ...
            var connection = $"Data Source={Path.Combine(directory, "judge151.db")};Pooling=False";
            ...
            builder.Services.AddDbContextFactory<LocalNodeDbContext>(options => options.UseSqlite(connection));

            var team = new Harborline.Api.Kernel.Runtime.Teams.TeamContext(
                new Harborline.Api.Kernel.Runtime.Teams.TeamId(
                    Guid.Parse("7e571510-0000-0000-0000-0000000000aa")),
                "Judge 151",
                new ServiceCollection().BuildServiceProvider(),
                TimeProvider.System);
            var activeTeam = new JudgeActiveTeam(team);
            builder.Services.AddSingleton<Harborline.Api.Kernel.Runtime.Teams.IActiveTeamAccessor>(activeTeam);

            var app = builder.Build();
            // The production listener binds the desktop-plane feature before the middleware; without it the
            // device-reachable product-data fence answers 403 before the route runs.
        ...
        private sealed class JudgeActiveTeam(Harborline.Api.Kernel.Runtime.Teams.TeamContext active)
            : Harborline.Api.Kernel.Runtime.Teams.IActiveTeamAccessor
        {
            public Harborline.Api.Kernel.Runtime.Teams.TeamContext? Active => active;

            public Task SetActiveAsync(
                Harborline.Api.Kernel.Runtime.Teams.TeamId teamId, CancellationToken ct) =>
                throw new NotSupportedException();

            public event EventHandler<
                Harborline.Api.Kernel.Runtime.Teams.ActiveTeamChangedEventArgs>? ActiveChanged
            {
                add { }
                remove { }
            }
