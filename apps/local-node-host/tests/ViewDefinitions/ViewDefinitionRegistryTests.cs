...
public sealed class ViewDefinitionRegistryTests
{
    [Fact]
    public async Task Register_requires_a_known_report_descriptor_before_persisting()
    {
        var registry = new InMemoryViewDefinitionRegistry(new DescriptorRegistry());
        ...
        Assert.Equal("view_definition.kind_not_registered", exception.ErrorCode);
    ...
    {
        var registry = new InMemoryViewDefinitionRegistry(new DescriptorRegistry("entity-table"));
        var original = Definition("tenant-a", "trial", "2.4.1", "entity-table", "{\"chartId\":\"chart-7\"}");
        ...
        Assert.Equal("view_definition.pinned_tuple_conflict", exception.ErrorCode);
    ...
    {
        var descriptors = new DescriptorRegistry("entity-table");
        var registry = new InMemoryViewDefinitionRegistry(descriptors, new TitleCanonicalizer());

        var stored = await registry.RegisterAsync(
            Definition("tenant-a", "trial", "1.0.0", "entity-table", "{\"chartId\":\"chart-7\"}"));
    ...
    public async Task ListVersions_orders_semver_descending_when_every_version_is_a_strict_triple()
    {
        ...
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.9.0", "entity-table", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "2.0.0", "entity-table", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.10.0", "entity-table", "{}"));
    ...
    public async Task ListVersions_falls_back_to_ordinal_descending_when_any_version_is_not_a_triple()
    {
        ...
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.9.0", "entity-table", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.10.0", "entity-table", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "2024-legacy", "entity-table", "{}"));
    ...
    {
        var registry = new InMemoryViewDefinitionRegistry(new DescriptorRegistry("entity-table"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.0.0", "entity-table", "{}"));
    ...
    public async Task ListDefinitions_returns_one_head_per_key_ordered_by_key_and_scoped_to_the_tenant()
    {
        ...
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.9.0", "entity-table", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.10.0", "entity-table", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "aging", "3.0.0", "entity-table", "{}"));
        await registry.RegisterAsync(Definition("tenant-b", "foreign", "9.9.9", "entity-table", "{}"));
    ...
    private sealed class DescriptorRegistry(params string[] registeredKinds)
        : IViewDefinitionDescriptorRegistry
    {
        ...
        public ValueTask AdmitAsync(ViewDefinition definition, CancellationToken cancellationToken = default)
        {
            ...
            if (!registeredKinds.Contains(definition.ViewKind, StringComparer.Ordinal))
            {
                throw new ViewDefinitionGovernanceException("view_definition.kind_not_registered");
            }
    ...
    private sealed class TitleCanonicalizer : IViewDefinitionCanonicalizer
    {
