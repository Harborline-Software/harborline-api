...
/// <summary>
/// Wave 5.2.D <see cref="IHostedService"/> that exposes the aggregate <c>/health</c>
/// endpoint and the dedicated <c>/live</c> and <c>/ready</c> probes. Wave 5.3.C refactor:
/// the endpoint no longer owns its own
/// <see cref="Microsoft.AspNetCore.Builder.WebApplication"/>; instead it
/// registers the health-probe mappings on the singleton
/// <see cref="SharedHostedWebApp"/>, which owns the shared Kestrel lifecycle
/// so the probes and <c>/ws</c> live behind the same listener.
/// </summary>
/// <remarks>
/// <para>
