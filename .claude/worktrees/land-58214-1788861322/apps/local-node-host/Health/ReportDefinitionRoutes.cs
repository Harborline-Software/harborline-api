...
/// <summary>
/// Node-local READ surface over the pack-projected report definition registry (ticket 085, the
/// first copy of the ticket-079 admin pattern's api half).
/// </summary>
/// <remarks>
...
/// </para>
/// </remarks>
public static class ReportDefinitionRoutes
{
    /// <summary>The route base every endpoint in this family lives under.</summary>
    public const string RouteBase = "/api/local-node/reports/definitions";

    /// <summary>Maps the read-only report definition routes onto <paramref name="app"/>.</summary>
    /// <param name="app">The route builder — hand this the desktop-plane-only group, never the bare application.</param>
    /// <param name="registry">The pack-projected report definition registry.</param>
        ...
        IActiveTeamAccessor activeTeam)
    {
        ...
        // GET /api/local-node/reports/definitions — one head revision per key.
        app.MapGet(RouteBase, async (CancellationToken ct) =>
        {
            ...
            return Results.Ok(heads.Select(ReportDefinitionSummaryDto.From).ToList());
        });

        // GET /api/local-node/reports/definitions/{key} — the head revision's detail.
        app.MapGet($"{RouteBase}/{{key}}", async (string key, CancellationToken ct) =>
        {
            ...
            if (history is null)
            {
                ...
                return Results.NotFound(new { code = "report_definition.not_found" });
            }

            return Results.Ok(ReportDefinitionDto.From(history.Versions[0]));
        });

        // GET /api/local-node/reports/definitions/{key}/versions — full history, newest first.
        app.MapGet($"{RouteBase}/{{key}}/versions", async (string key, CancellationToken ct) =>
        {
            ...
            return Results.Ok(ReportDefinitionVersionListDto.From(history));
        });

        // GET /api/local-node/reports/definitions/{key}/versions/{version} — one pinned revision.
        app.MapGet($"{RouteBase}/{{key}}/versions/{{version}}", async (string key, string version, CancellationToken ct) =>
        {
            ...
            {
                return Results.NotFound(new { code = "report_definition.version_not_found" });
            }

            return Results.Ok(ReportDefinitionDto.From(definition));
        });
    }
}
...
public sealed record ReportDefinitionSummaryDto(
    ...
    [property: JsonPropertyName("reportKind")] string ReportKind,
    [property: JsonPropertyName("cascadeLayer")] string CascadeLayer)
{
    ...
    public static ReportDefinitionSummaryDto From(ReportDefinition definition) => new(
        definition.Key,
        definition.Version,
        definition.Title,
    ...
    [property: JsonPropertyName("provenance")] JsonElement Provenance)
{
    ...
    public static ReportDefinitionDto From(ReportDefinition definition) => new(
        definition.Key,
        definition.Version,
        definition.Title,
        definition.ReportKind,
...
public sealed record ReportDefinitionVersionListDto(
    [property: JsonPropertyName("ordering")] string Ordering,
    [property: JsonPropertyName("versions")] IReadOnlyList<ReportDefinitionSummaryDto> Versions)
{
    /// <summary>Projects the history wire shape from the registry's ordered result.</summary>
    public static ReportDefinitionVersionListDto From(ReportDefinitionVersionList history) => new(
        history.Ordering,
        history.Versions.Select(ReportDefinitionSummaryDto.From).ToList());
}
