...
/// <summary>
/// HTTP-contract coverage for the read-only report definition route family (ticket 086), mapped
...
/// </summary>
public sealed class ViewDefinitionRouteTests : IAsyncLifetime
{
    ...
    private InMemoryViewDefinitionRegistry _registry = null!;
    ...
    public async Task InitializeAsync()
    {
        ...
        _registry = new InMemoryViewDefinitionRegistry(new AcceptAllDescriptorRegistry());
        ...
        ViewDefinitionRoutes.Map(_app.MapDesktopPlaneOnlyGroup(), _registry, _activeTeam);
    ...
    {
        var rows = await _client.GetFromJsonAsync<JsonElement>(ViewDefinitionRoutes.RouteBase);
    ...
    {
        var detail = await _client.GetFromJsonAsync<JsonElement>($"{ViewDefinitionRoutes.RouteBase}/trial");

        Assert.Equal("1.10.0", detail.GetProperty("version").GetString());
        Assert.Equal("table", detail.GetProperty("viewKind").GetString());
        ...
        Assert.Equal(HttpStatusCode.NotFound,
            (await _client.GetAsync($"{ViewDefinitionRoutes.RouteBase}/foreign")).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound,
            (await _client.GetAsync($"{ViewDefinitionRoutes.RouteBase}/unknown")).StatusCode);
    }
    ...
    public async Task Every_404_carries_a_machine_code_and_no_English()
    {
        ...
        var unknownKey = await ReadBodyAsync($"{ViewDefinitionRoutes.RouteBase}/unknown");
        var unknownVersions = await ReadBodyAsync($"{ViewDefinitionRoutes.RouteBase}/unknown/versions");
        var unknownVersion = await ReadBodyAsync($"{ViewDefinitionRoutes.RouteBase}/trial/versions/0.0.0-absent");

        Assert.Equal("view_definition.not_found", unknownKey.GetProperty("code").GetString());
        Assert.Equal("view_definition.not_found", unknownVersions.GetProperty("code").GetString());
        Assert.Equal("view_definition.version_not_found", unknownVersion.GetProperty("code").GetString());
    ...
    public async Task Versions_lists_newest_first_with_the_ordering_rule_named()
    {
        var history = await _client.GetFromJsonAsync<JsonElement>(
            $"{ViewDefinitionRoutes.RouteBase}/trial/versions");
    ...
    public async Task One_version_is_an_exact_pinned_read_with_404_on_absent_tuples()
    {
        var revision = await _client.GetFromJsonAsync<JsonElement>(
            $"{ViewDefinitionRoutes.RouteBase}/trial/versions/1.9.0");

        Assert.Equal("1.9.0", revision.GetProperty("version").GetString());
        Assert.Equal(HttpStatusCode.NotFound,
            (await _client.GetAsync($"{ViewDefinitionRoutes.RouteBase}/trial/versions/0.0.1")).StatusCode);
    }
    ...
    {
        using var request = new HttpRequestMessage(HttpMethod.Get, ViewDefinitionRoutes.RouteBase);
    ...
    private static ViewDefinition Definition(string tenant, string key, string version, string title)
    {
