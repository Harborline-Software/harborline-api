...
public sealed class ReportDefinitionRegistryTests
{
    [Fact]
    public async Task Register_requires_a_known_report_descriptor_before_persisting()
    {
        var registry = new InMemoryReportDefinitionRegistry(new DescriptorRegistry());
        var definition = Definition("tenant-a", "cash-position", "1.0.0", "unknown-kind", "{\"days\":30}");

        var exception = await Assert.ThrowsAsync<ReportDefinitionGovernanceException>(
            () => registry.RegisterAsync(definition).AsTask());

        Assert.Equal("report_definition.kind_not_registered", exception.ErrorCode);
    ...
    {
        var registry = new InMemoryReportDefinitionRegistry(new DescriptorRegistry("trial-balance"));
        var original = Definition("tenant-a", "trial", "2.4.1", "trial-balance", "{\"chartId\":\"chart-7\"}");
        ...
        Assert.Equal("report_definition.pinned_tuple_conflict", exception.ErrorCode);
    ...
    {
        var descriptors = new DescriptorRegistry("trial-balance");
        var registry = new InMemoryReportDefinitionRegistry(descriptors, new TitleCanonicalizer());

        var stored = await registry.RegisterAsync(
            Definition("tenant-a", "trial", "1.0.0", "trial-balance", "{\"chartId\":\"chart-7\"}"));
    ...
    public async Task ListVersions_orders_semver_descending_when_every_version_is_a_strict_triple()
    {
        ...
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.9.0", "trial-balance", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "2.0.0", "trial-balance", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.10.0", "trial-balance", "{}"));
    ...
    public async Task ListVersions_falls_back_to_ordinal_descending_when_any_version_is_not_a_triple()
    {
        ...
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.9.0", "trial-balance", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.10.0", "trial-balance", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "2024-legacy", "trial-balance", "{}"));
    ...
    {
        var registry = new InMemoryReportDefinitionRegistry(new DescriptorRegistry("trial-balance"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.0.0", "trial-balance", "{}"));
    ...
    public async Task ListDefinitions_returns_one_head_per_key_ordered_by_key_and_scoped_to_the_tenant()
    {
        ...
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.9.0", "trial-balance", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "trial", "1.10.0", "trial-balance", "{}"));
        await registry.RegisterAsync(Definition("tenant-a", "aging", "3.0.0", "trial-balance", "{}"));
        await registry.RegisterAsync(Definition("tenant-b", "foreign", "9.9.9", "trial-balance", "{}"));
    ...
    private sealed class DescriptorRegistry(params string[] registeredKinds)
        : IReportDefinitionDescriptorRegistry
    {
        ...
        public ValueTask AdmitAsync(ReportDefinition definition, CancellationToken cancellationToken = default)
        {
            ...
            if (!registeredKinds.Contains(definition.ReportKind, StringComparer.Ordinal))
            {
                throw new ReportDefinitionGovernanceException("report_definition.kind_not_registered");
            }
    ...
    private sealed class TitleCanonicalizer : IReportDefinitionCanonicalizer
    {
