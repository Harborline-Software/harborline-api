...
namespace Harborline.Api.UIAdapters.Blazor.Components.Layout.ResizableContainer;
...
/// </summary>
public partial class HarborlineResizableContainer : HarborlineComponentBase, IAsyncDisposable
{
    // ── Parameters: Content ────────────────────────────────────────────
    ...
    [Parameter] public EventCallback<HarborlineResizeEventArgs> OnResizeStart { get; set; }

    /// <summary>Fires on each frame during a drag resize.</summary>
    [Parameter] public EventCallback<HarborlineResizeEventArgs> OnResizing { get; set; }

    /// <summary>Fires when a drag resize ends.</summary>
    [Parameter] public EventCallback<HarborlineResizeEventArgs> OnResizeEnd { get; set; }

    /// <summary>Fires when ResizeObserver detects a size change (any cause).</summary>
    [Parameter] public EventCallback<HarborlineObservedSizeChangedEventArgs> OnObservedSizeChanged { get; set; }
    ...
    private DotNetObjectReference<HarborlineResizableContainer>? _dotNetRef;
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            ...
            _jsModule = await JS.InvokeAsync<IJSObjectReference>(
                "import", HarborlineJsModuleLoader.Resolve("js/resizable-container.js"));
    ...
    /// container as dragging and forwards a <see cref="HarborlineResizeEventArgs"/> to <c>OnResizeStart</c>.
    /// </summary>
    ...
    {
        var args = new HarborlineObservedSizeChangedEventArgs
        {
            Width = $"{widthPx}px",
            Height = $"{heightPx}px",
            WidthPixels = widthPx,
    ...
    private static HarborlineResizeEventArgs CreateResizeArgs(
        double widthPx, double heightPx, ResizeEdges activeEdge, bool isUserInitiated)
    {
        return new HarborlineResizeEventArgs
        {
            Width = $"{widthPx}px",
            Height = $"{heightPx}px",
            WidthPixels = widthPx,
