...
Base class (`HarborlineComponentBase`) provides: `Class` (string?), `Style` (string?), `AdditionalAttributes`.

---

## 1. HarborlineProgressBar.razor vs `progressbar/overview.md`
...
## 2. HarborlineProgressCircle.razor vs `loader/overview.md`

The spec describes a generic `HarborlineLoader` component; the implementation is a circular progress indicator. These are architecturally different components.

| Area | Spec | Implementation | Severity |
|------|------|----------------|----------|
| Component name | `HarborlineLoader` | `HarborlineProgressCircle` -- different concept | **[Low]** (intentional split) |
...
## 3. HarborlineSkeleton.razor vs `skeleton/overview.md`
...
## 4. HarborlineSnackbar.razor -- NO SPEC
...
## 5. HarborlineSnackbarHost.razor -- NO SPEC

Implemented features (notification host/container):
- Injects `IHarborlineNotificationService` for centralized notification management
...
## 6. HarborlineSpinner.razor vs `loader/overview.md`

The spec describes `HarborlineLoader`; this is a simpler spinner variant.
...
## 7. HarborlineToast.razor vs `notification/overview.md`

The spec describes a notification component with a service-oriented `Show/Hide` API; the implementation is a simple presentational component.
...
**Note:** The spec's notification functionality is partially covered by `HarborlineSnackbarHost` (stacking, positioning, auto-close, service integration) rather than `HarborlineToast`. The Toast component appears to be a low-level building block.
...
|-----------|------|--------|-----|--------|
| HarborlineProgressBar | 3 | 1 | 0 | Significant gaps (Max, Label, ARIA) |
| HarborlineProgressCircle | 0 | 2 | 3 | Intentionally different from Loader spec |
| HarborlineSkeleton | 1 | 2 | 0 | Missing AnimationType and Visible |
| HarborlineSnackbar | 0 | 0 | 2 | No spec (functional) |
| HarborlineSnackbarHost | 0 | 0 | 1 | No spec (well-implemented) |
| HarborlineSpinner | 0 | 3 | 1 | Missing Visible, ThemeColor, ARIA |
| HarborlineToast | 3 | 4 | 1 | Major architectural divergence from spec |

**Top priorities:** HarborlineProgressBar (add `Max`, label, ARIA) and HarborlineToast (decide whether spec's imperative API is needed or if SnackbarHost covers it).
