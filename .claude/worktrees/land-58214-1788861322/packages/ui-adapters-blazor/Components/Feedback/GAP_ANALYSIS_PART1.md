...
## 1. HarborlineAlert.razor vs Notification Spec

The spec describes a notification component with imperative `Show()`/`Hide()`/`HideAll()` methods and a `NotificationModel` class. `HarborlineAlert` is a declarative inline alert -- a fundamentally different pattern. This is intentional (alert vs toast notification), but there are gaps within what HarborlineAlert does implement.
...
## 2. HarborlineAlertStrip.razor (No Spec)
...
## 3. HarborlineCallout.razor (No Spec)
...
## 4. HarborlineConfirmDialog.razor vs Dialog Spec
...
## 5. HarborlineDataBanner.razor (No Spec)
...
## 6. HarborlineDataToast.razor (No Spec)
...
## 7. HarborlineDialog.razor vs Dialog Spec

The closest match to the spec. Covers more ground than HarborlineConfirmDialog.
...
|-----------|-----------|---------------|
| HarborlineAlert | Partial (different pattern) | No icons, no animation, no ThemeColor |
| HarborlineAlertStrip | No spec | Missing ARIA, no dismiss per item |
| HarborlineCallout | No spec | Missing ARIA roles |
| HarborlineConfirmDialog | Weak | No two-way `Visible` binding, string-only content, no dimensions |
| HarborlineDataBanner | No spec | Missing ARIA, hardcoded text |
| HarborlineDataToast | No spec | No auto-dismiss, no animation, missing ARIA live region |
| HarborlineDialog | Moderate | No two-way `Visible` binding, no close button, no custom button RenderFragment |

**Top priority fixes (High severity):**
1. Both `HarborlineDialog` and `HarborlineConfirmDialog` should adopt `Visible` with two-way binding (`@bind-Visible`) per the Dialog spec, replacing the one-way `IsOpen` parameter.
