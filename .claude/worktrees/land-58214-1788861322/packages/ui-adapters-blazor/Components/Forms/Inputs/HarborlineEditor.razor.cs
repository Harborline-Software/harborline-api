...
namespace Harborline.Api.UIAdapters.Blazor.Components.Forms.Inputs.MvpEditor;
...
/// <c>Harborline.Api.UIAdapters.Blazor.Components.Editors.HarborlineEditor</c>, which
/// is not bound to the deprecated <c>execCommand</c> API. Both may coexist — this one lives in a
/// nested namespace that <c>_Imports.razor</c> does not import, so consumers
/// that import <c>Harborline.Api.UIAdapters.Blazor.Components.Editors</c> are
/// unaffected by it.
/// </para>
///
/// <para>
...
/// </para>
/// </summary>
public partial class HarborlineEditor : HarborlineComponentBase
{
    ...
    private string SurfaceStyle()
    {
        ...
        // Ticket 267: this read named `--harborline-border`, which no stylesheet, component or
        // theme service in this package (or any sibling repository) declares — every render hit
        // the literal fallback. The literal stands alone now; the tokens HarborlineThemeProvider
        // actually emits are the `--sf-*` family, and a host that wants to theme this border sets
        // one of those (or its own property) through the provider's CssVariables parameter.
        parts.Add("border:1px solid #d1d5db");
