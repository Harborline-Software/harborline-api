...
- **Not generic**: the spec's numeric text box is generic over numeric types (`int`, `decimal`, `double`, etc.); implementation is hardcoded to `double`.
...
- **Not a RadioGroup**: Spec describes a `HarborlineRadioGroup` that renders a full list from `Data` with `TextField`/`ValueField` binding; implementation is a single radio button requiring manual repetition.
...
## 8. HarborlineSearchBox

**Current**: `Value`/`ValueChanged`, `Placeholder`, `OnSearch`, `OnEscape`, `Disabled`, `KbdHint`. Wraps `HarborlineTextField` with search icon prefix, clear button, and kbd hint.
