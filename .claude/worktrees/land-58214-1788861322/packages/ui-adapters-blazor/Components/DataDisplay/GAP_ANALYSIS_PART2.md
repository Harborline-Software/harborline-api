...
> **Note (2026-04-03):** `HarborlineTable` has been removed as an obsolete component. It was superseded by `HarborlineDataGrid`. Historical references to `HarborlineTable` in this document are retained for audit accuracy.

Base class (`HarborlineComponentBase`) provides: `Class`, `Style`, `AdditionalAttributes`.

---

## 1. HarborlineList.razor (spec: listbox/overview.md)
...
## 2. HarborlineListItem.razor (part of listbox spec)
...
| No integration with parent HarborlineList for selection/reorder | **[High]** |

**Summary:** Purely structural. The spec expects the ListBox to render items from data, not via manual `<HarborlineListItem>` children. The entire item-rendering model differs from spec.

---

## 3. HarborlineListView.razor (spec: listview/overview.md)
...
## 4. HarborlinePopover.razor (spec: popover/overview.md)
...
| `Width` / `Height` parameters | **[Low]** |
| Rendering as child of a spec-level root component (portal pattern) | **[Medium]** |
...
## 5. HarborlineTable.razor (no spec)
...
## 6. HarborlineTimeline.razor (no spec)

**Implemented:**
- `ChildContent` -- renders a `<div>` container with timeline CSS class.

**Summary:** Simple container for `HarborlineTimelineItem` children. No orientation, alignment mode, or reverse parameters.

---

## 7. HarborlineTimelineItem.razor (no spec)
...
## 8. HarborlineTooltip.razor (spec: tooltip/overview.md)
...
## 9. HarborlineTypography.razor (no spec)
...
|----------|-----------|------------|
| **Critical** | HarborlineList/HarborlineListItem | Rewrite as data-driven ListBox with selection, toolbar, drag-drop |
| **High** | HarborlinePopover | Add anchor selector model, programmatic Show/Hide, header/actions |
| **High** | HarborlineTooltip | Add TargetSelector for external/multi-target attachment |
| **Moderate** | HarborlineListView | Add header/footer templates, external page control |
| **Low** | HarborlineTable, HarborlineTimeline, HarborlineTimelineItem, HarborlineTypography | Adequate for current scope; enhance as needed |
