# HarborlineDataGrid — Phased Gap Analysis & Implementation Tracker
...
- HarborlineGridCommandButton component (Add/Edit/Save/Cancel/Delete + custom commands)
...
### Architecture
```
HarborlineDataGrid.razor           — Markup (~240 lines)
HarborlineDataGrid.razor.cs        — Parameters, state, lifecycle (~300 lines)
HarborlineDataGrid.Rendering.cs    — RenderTreeBuilder: rows, cells, filter menu (~360 lines)
HarborlineDataGrid.Data.cs         — Filter/sort/page pipeline, event handlers (~430 lines)
HarborlineDataGrid.Editing.cs      — CRUD operations, detail expansion (~130 lines)
HarborlineGridColumn.razor         — Column definition component
HarborlineGridCommandButton.razor  — Command button component
HarborlineGridToolbar.razor        — Toolbar container
...
- [ ] A1.1 — Add `Groupable` parameter to `HarborlineGridColumn` (default: true)
...
- [x] A2.4 — Allow explicit `HarborlineGridColumn` children to override auto-generated columns
...
- [x] A5.1 — Add `Editable` bool parameter to `HarborlineGridColumn` (default: true)
- [x] A5.2 — Respect `Editable` in Rendering.cs — skip EditorTemplate for non-editable columns
- [x] A5.3 — Add `HeaderClass` string parameter to `HarborlineGridColumn`
- [x] A5.4 — Add `Id` string parameter to `HarborlineGridColumn`
- [x] A5.5 — Add `ShowColumnMenu` bool parameter to `HarborlineGridColumn` (default: false)
...
Note: Radzen uses a `DataGridSettings` object with `SaveSettings()`/`LoadSettings()` for full state round-tripping. Harborline uses event-based `OnStateChanged` instead; a `SaveSettings()`/`LoadSettings()` helper is deferred as a potential A6.7+ follow-up task (not implemented here — event-based approach is sufficient for round-tripping).
...
- [x] B0.2 — Register JS module in `HarborlineDataGrid` via `IJSRuntime.InvokeAsync<IJSObjectReference>("import", ...)`
- [x] B0.3 — Add `IAsyncDisposable` implementation to dispose JS module reference
- [x] B0.4 — Add `[Inject] IJSRuntime JS { get; set; }` to HarborlineDataGrid.razor.cs
...
- [x] B2.2 — Add `Resizable` bool parameter to `HarborlineGridColumn` (default: true when grid allows)
...
- [x] B3.2 — Add `Reorderable` bool parameter to `HarborlineGridColumn` (default: true when grid allows)
- [x] B3.3 — Add `OrderIndex` (int?) property to `HarborlineGridColumn` for persistence
...
- [x] B5.1 — Add `Locked` bool parameter to `HarborlineGridColumn` (default: false)
...
- [x] B5.3 — Add `Lockable` bool parameter to `HarborlineGridColumn` (default: true)
...
- [x] C3.1 — Create `HarborlineGridColumnMenu` component (dropdown popup)
...
- [ ] C4.1 — Create a column-chooser component (dialog/popup)
...
- [ ] C6.1 — Create a column-group component
...
- [ ] D1.1 — Define the AI-service interface
