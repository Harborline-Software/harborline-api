...
/// <summary>
/// Represents one rendered group in a grouped <see cref="HarborlineListView{TItem}"/>.
/// </summary>
...
/// </summary>
/// <typeparam name="TItem">The list item type.</typeparam>
public partial class HarborlineListView<TItem> : HarborlineComponentBase
{
    // ── Data ────────────────────────────────────────────────────────────────
    ...
    /// <summary>
    /// View-local presentation-role mapping. <see cref="ShapeRoleMapping.GroupedBy"/> takes
    /// precedence over the legacy grouping parameters, which remain compatibility fallbacks.
    /// </summary>
    ...
    {
        if (!string.IsNullOrWhiteSpace(ShapeRoles?.GroupedBy))
        {
            return new List<GroupDescriptor> { new() { Field = ShapeRoles.GroupedBy } };
        }
