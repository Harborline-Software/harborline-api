# HarborlineGantt Implementation Notes
...
- ~~**Filter checkbox list**~~: E12 — Drawer-hosted + E16 HarborlinePopup option

### Popup/Overlay Gaps — RESOLVED in Pass 3
- ~~**Popup edit mode**~~: E15 — GanttTreeListEditMode.Popup using HarborlinePopup
- ~~**Filter popup anchoring**~~: E16 — GanttFilterPopupMode.Popup using HarborlinePopup
- ~~**Column chooser**~~: E17 — ShowColumnChooser + HarborlinePopup panel
...
- `HarborlineGanttDependencies<TItem>` child component using CascadingParameter to find parent.
...
### E14 — HarborlinePopup Primitive
- Stub implementation: absolute positioning, no JS scroll tracking.
- Inline JS eval for outside-click detection (same pattern as HarborlineWindow).
...
- `BeginPopupEdit(rowIdx, field)` opens HarborlinePopup with form for all editable columns.
...
- Popup mode uses HarborlinePopup anchored to filter button (id="mar-gantt-filter-{field}").
...
- Added `_insertedItem` and `_parentItem` private fields to HarborlineGantt.
...
- Added `IdField`, `PredecessorIdField`, `SuccessorIdField`, `TypeField` parameters to `HarborlineGanttDependencies`.
