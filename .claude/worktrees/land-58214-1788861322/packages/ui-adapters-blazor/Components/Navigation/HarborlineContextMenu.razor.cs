...
public partial class HarborlineContextMenu : HarborlineComponentBase
{
    ...
    /// <summary>Field name for a destructive/danger item flag.</summary>
    [Parameter] public string DangerField { get; set; } = "Danger";
    ...
    internal RenderFragment RenderItems(IEnumerable<object> items) => builder =>
    {
        ...
        foreach (var item in items)
        {
            ...
            var isDanger = GetBool(item, DangerField);
            ...
            else
            {
                ...
                builder.AddAttribute(22, "class", CssProvider.MenuItemClass(isDisabled)
                    + (isDanger ? " mar-context-menu__item--danger" : "") + focusedClass);
