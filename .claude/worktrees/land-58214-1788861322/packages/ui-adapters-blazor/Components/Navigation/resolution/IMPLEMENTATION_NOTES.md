...
## HarborlineTreeView — Source Review Design Decisions
...
- HTML `<input type="checkbox">` requires JS interop for `indeterminate` property (no Blazor-native way to set it), OR use a CSS-only approach with `:indeterminate` pseudo-class via a `HarborlineCheckbox` sub-component
