// Harborline a11y primitives — live announcer + focus trap
// Bridges ILiveAnnouncer and IFocusTrap Blazor adapters to the browser.
// Loaded lazily as an ES module per the Harborline JS module convention.
...
// ── Example-panel tabs ─────────────────────────────────────────────────────
//
// Blazor event callbacks cannot synchronously prevent the browser's default
// keyboard action. This listener runs in the browser so Arrow/Home/End can
// change tabs without also scrolling the page. Tab is deliberately excluded:
// the tab strip must never become a keyboard trap.

const _exampleTabNavigationHandlers = new WeakMap();
const EXAMPLE_TAB_NAVIGATION_KEYS = new Set(['ArrowLeft', 'ArrowRight', 'Home', 'End']);

export function preventExampleTabNavigationScroll(tabList) {
    if (!tabList || _exampleTabNavigationHandlers.has(tabList)) return;

    const onKeyDown = (event) => {
        if (EXAMPLE_TAB_NAVIGATION_KEYS.has(event.key)) event.preventDefault();
    };

    tabList.addEventListener('keydown', onKeyDown);
    _exampleTabNavigationHandlers.set(tabList, onKeyDown);
}

export function removeExampleTabNavigationScroll(tabList) {
    const onKeyDown = _exampleTabNavigationHandlers.get(tabList);
    if (!onKeyDown) return;

    tabList.removeEventListener('keydown', onKeyDown);
    _exampleTabNavigationHandlers.delete(tabList);
}
