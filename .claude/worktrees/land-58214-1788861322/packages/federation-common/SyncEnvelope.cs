...
namespace Harborline.Api.Federation.Common;
    ...
    Signature Signature)
{
    ...
    public static SyncEnvelope SignAndCreate(
        ...
        ReadOnlyMemory<byte> payload,
        DateTimeOffset at)
        => EnvelopeSigning.Sign(signer, toPeer, kind, payload, at);
...
public enum SyncMessageKind
{
    ...
    /// <summary>
    /// A Tauri client's encrypted CRDT delta for an operational doctype.
    /// The <see cref="SyncEnvelope.Payload"/> is an opaque ciphertext blob —
    /// the relay routes it without deserialising or interpreting its content
    /// (fence F-2, ADR 0113 B1). The client encrypts the delta under its
    /// per-tenant DEK before placing it here; only a holder of the DEK can
    /// recover the plaintext CRDT update bytes.
    /// </summary>
    /// <remarks>
    /// Adding this variant is a routing discriminator only. No .NET kernel or
    /// Bridge code EVER parses the payload bytes as a CRDT engine type — doing
    /// so would violate fence F-2 and constitute a breaking change to ADR 0113.
    /// </remarks>
    ClientCrdtDelta,
}
