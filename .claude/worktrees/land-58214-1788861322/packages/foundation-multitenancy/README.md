# Harborline.Api.Foundation.MultiTenancy

Tenancy primitives — `TenantMetadata`, `ITenantContext`, `ITenantResolver`, `ITenantCatalog`, scoped markers, and an in-memory default.

Does NOT reference Finbuckle — Bridge handles that adaptation in its host. Implements [ADR 0008](../../docs/adrs/0008-foundation-multitenancy.md).
...
- **`TenantId`** — string-wrapper record struct from `Harborline.Api.Foundation.Assets.Common` (sentinel: `TenantId.Default`).

### Default impls

- **In-memory `ITenantCatalog`** — for tests + non-production hosts.
- Helper extensions for tenant-scoped query filters in `Harborline.Api.Foundation.Persistence`.
...
- [ADR 0008](../../docs/adrs/0008-foundation-multitenancy.md) — multi-tenancy + global query filter pattern

## See also

- [apps/docs Overview](../../apps/docs/foundation/multitenancy/overview.md)
- [Harborline.Api.Foundation.Persistence](../foundation-persistence/README.md) — consumer (`IMustHaveTenant` query filter)
