...
namespace Harborline.Api.Foundation.MissionSpace.Regulatory.Audit;
...
public sealed class RegulatoryAuditEmitter
{
    ...
    public Task EmitAsync(AuditEventType eventType, AuditPayload payload, CancellationToken ct)
        => EmitInternalAsync(eventType, payload, _time.GetUtcNow(), ct);
        ...
        CancellationToken ct)
    {
        ...
        if (!ShouldEmit(dedupKey, _ruleDedupWindow, occurredAt)) return false;
        await EmitInternalAsync(eventType, payload, occurredAt, ct).ConfigureAwait(false);
        return true;
    }
    ...
    {
        var occurredAt = _time.GetUtcNow();
        if (!ShouldEmit(dedupKey, _regimeDedupWindow, occurredAt)) return false;
        await EmitInternalAsync(eventType, payload, occurredAt, ct).ConfigureAwait(false);
        return true;
    }

    private bool ShouldEmit(string dedupKey, TimeSpan window, DateTimeOffset now)
    {
        var fire = false;
        _dedup.AddOrUpdate(
            dedupKey,
