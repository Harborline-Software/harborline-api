...
using Harborline.Api.Blocks.FinancialPeriods.Financial;
...
namespace Harborline.Api.Blocks.FinancialPeriods.Services;
...
public sealed class FiscalYearCloseService : IFiscalYearCloseService
{
    ...
    {
        await RequireAuthorityAsync(fyId, authority, cancellationToken).ConfigureAwait(false);
        ...
        {
            var soft = await _periodClose.SoftCloseAsync(
                p.Id, authority, cancellationToken: cancellationToken).ConfigureAwait(false);
            if (!soft.IsSuccess)
                return new FiscalYearCloseResult(fy, null,
        ...
        if (closingEntryId is null && closingLines.Count > 0)
        {
            ...
            var postResult = await _posting.PostAsync(closingEntry, authority, cancellationToken).ConfigureAwait(false);
        ...
        foreach (var p in periods)
        {
            ...
            var lockResult = await _periodClose.LockAsync(p.Id, authority, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var unlock = await _periodClose.UnlockAsync(
                p.Id, $"FY reopen: {auditMemo}", authority, cancellationToken).ConfigureAwait(false);
        ...
        string? correlationId = null)
    {
        ...
        {
            EventId              = Guid.CreateVersion7(occurredAt).ToString(),
            EventType            = eventType,
