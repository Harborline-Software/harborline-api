...
public sealed class ErpnextPartyImporter : IErpnextPartyImporter
{
    ...
    public ErpnextPartyImporter(IPartyReadModel read, IPartyWriteService write, TimeProvider time)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (existing is null)
        {
            party = await _write.CreateAsync(
            ...
            await TryAttachContactsAsync(party.Id, email, phoneE164, actor, at, cancellationToken).ConfigureAwait(false);
            await _write.AttachRoleAsync(party.Id, roleName, roleRecordId, actor, at, cancellationToken)
                .ConfigureAwait(false);
