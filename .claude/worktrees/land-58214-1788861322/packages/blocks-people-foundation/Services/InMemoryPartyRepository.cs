...
public sealed class InMemoryPartyRepository : IPartyReadModel, IPartyWriteService
{
    ...
    public InMemoryPartyRepository(TimeProvider timeProvider, IDomainEventPublisher? events = null)
    {
    ...
    public async Task<Party> UpdateAsync(
        Party updated, PartyId actor, DateTimeOffset at, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public Task<Party> DeleteAsync(
        PartyId id, string? reason, PartyId actor, DateTimeOffset at, CancellationToken cancellationToken = default)
    {
    ...
    private async Task<TReturn> PublishAndReturn<TPayload, TReturn>(
        ...
        CancellationToken cancellationToken)
    {
        await Publish(eventType, payload, idempotencyKey, tenantId, at, cancellationToken).ConfigureAwait(false);
        return returnValue;
    }
}
