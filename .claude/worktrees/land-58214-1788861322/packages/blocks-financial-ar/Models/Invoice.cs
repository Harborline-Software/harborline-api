...
public sealed record Invoice : IMustHaveTenant
{
    ...
    /// <summary>
    /// The subsidiary-ledger account this invoice belongs to (ADR 0120 PR-C / C-ARCH-2).
    /// Additive, nullable, back-compat: null for legacy invoices that predate the
    /// sub-ledger primitive. The projection repoint filters on this FK when non-null,
    /// eliminating the PartyId-overmatch under per-lease granularity (a party with
    /// 2+ leases → 2+ accounts, all same PartyId+AR-control). Migration backfill sets
    /// this for all existing open items.
    /// </summary>
    public SubLedgerAccountId? SubLedgerAccountId { get; init; }
    ...
    public static Invoice Create(
        ...
        SubLedgerAccountId? subLedgerAccountId = null)
    {
        ...
        return new Invoice
        {
            ...
            // ADR 0120 PR-C / ADR 0122 §D4 P2 → (b): stamp the sub-ledger FK at create time so the
            // ISubLedgerReadModel projection (which filters FK-authoritatively via
            // ListBySubLedgerAccountAsync) resolves this invoice into the lease's history offline. Null
            // for non-lease invoices (back-compat — the FK is additive/nullable per Invoice.cs:103).
