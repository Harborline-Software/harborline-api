...
using Harborline.Api.Foundation.Transport.Audit;
    ...
    public async Task<IPeerTransport> SelectAsync(PeerId peer, CancellationToken ct)
    {
        ...
        if (_tier1 is not null && _tier1.IsAvailable)
        {
            ...
            if (resolved is not null)
            {
                ...
                await EmitAsync(AuditEventType.TransportTierSelected,
                    TransportAuditPayloads.TierSelected(peer, TransportTier.LocalNetwork, adapterName: null), attemptAt, ct).ConfigureAwait(false);
                return _tier1;
            }
        }
        ...
        foreach (var t2 in _tier2)
        {
            ...
            if (resolved is not null)
            {
                ...
                await EmitAsync(AuditEventType.TransportTierSelected,
                    TransportAuditPayloads.TierSelected(peer, TransportTier.MeshVpn, adapterName), attemptAt, ct).ConfigureAwait(false);
                return t2;
            }
            await EmitAsync(AuditEventType.MeshTransportFailed,
                TransportAuditPayloads.MeshTransportFailed(peer, adapterName ?? string.Empty, "resolve-miss"), attemptAt, ct).ConfigureAwait(false);
        }
        ...
        await EmitAsync(AuditEventType.TransportFallbackToRelay,
            TransportAuditPayloads.TransportFallbackToRelay(peer, t3Resolved is null ? "Failed" : "Selected"), attemptAt, ct).ConfigureAwait(false);
        return _tier3;
    }
