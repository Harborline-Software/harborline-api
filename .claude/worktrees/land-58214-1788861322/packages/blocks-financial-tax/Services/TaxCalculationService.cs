        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (code is null)
        {
            return Fail(input, TaxCalculationError.TaxCodeNotFound,
                $"TaxCodeId {input.TaxCodeId} not found.", taxCodeVersion: 0, calculatedAt);
        }
        ...
        if (applicableRates.Count == 0)
        {
            return Fail(input, TaxCalculationError.NoApplicableRates,
                ...
                taxCodeVersion: code.Version, calculatedAt);
        }
        ...
        {
            TaxApplication.OnSubtotal => ApplyOnSubtotal(input, withJur, code.Version, calculatedAt),
            TaxApplication.Compound => ApplyCompound(input, withJur, code.Version, calculatedAt),
            TaxApplication.Inclusive => InclusiveGuard(input, withJur, code, calculatedAt)
                ?? ApplyInclusive(input, withJur, code.Version, calculatedAt),
