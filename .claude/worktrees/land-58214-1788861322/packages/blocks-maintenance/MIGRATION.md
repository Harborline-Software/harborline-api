# Harborline.Api.Blocks.Maintenance — Migration Guide
...
The legacy `VendorSpecialty` enum is preserved for the migration window. Use `VendorSpecialtyClassifications.FromLegacyEnum(VendorSpecialty)` or `.ToList(VendorSpecialty)` for mechanical migration of existing call-sites. Each enum value maps 1:1 to a root taxonomy node in `Harborline.Vendor.Specialties@1.0.0` (e.g., `VendorSpecialty.Plumbing` → `plumbing`).
