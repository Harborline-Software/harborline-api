...
namespace Harborline.Api.Kernel.Ledger.Periods;
...
public sealed class PeriodCloser : IPeriodCloser
{
    ...
    /// <inheritdoc />
    public async Task<PeriodCloseResult> CloseAsync(DateTimeOffset periodEnd, CancellationToken ct)
    {
        ...
        // Durably persist the close event (ADR 0115 §D4) BEFORE mutating the in-memory boundary, so a
        // restart rebuilds LastClosedPeriodEnd from the durable log and the closed-period boundary
        // survives the crash. Order mirrors the posting hot-path: durable append first, then state.
        await _engine.PublishPeriodClosedAsync(closedEvent, ct).ConfigureAwait(false);
...
/// <see cref="Harborline.Api.Kernel.Ledger.PostingEngine"/> (reader — through
