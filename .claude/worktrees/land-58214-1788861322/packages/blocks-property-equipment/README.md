# Harborline.Api.Blocks.PropertyEquipment
...
Renamed from `blocks-property-assets` per UPF Rule 4 (2026-04-28) to disambiguate from foundation-tier `Harborline.Api.Foundation.Assets.Common`. The cluster's physical-equipment entity is now named `Equipment` (industry-standard for facilities management).

## What this ships

### Models

- **`Equipment`** — root entity (`IMustHaveTenant`); FK to `Harborline.Api.Blocks.Properties.Property` via `PropertyId`.
...
- **`PropertyEquipmentEntityModule`** — `IHarborlineEntityModule` contribution.
...
- [Harborline.Api.Blocks.Properties](../blocks-properties/README.md) — cluster spine
- [Harborline.Api.Blocks.Maintenance](../blocks-maintenance/README.md) — work-order consumer
