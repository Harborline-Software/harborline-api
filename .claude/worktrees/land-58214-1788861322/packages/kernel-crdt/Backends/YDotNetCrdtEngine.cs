...
// ============================================================================
//  ADR 0008 standardizes the CRDT engine on the Yjs/yrs family. YDotNet is the
//  sole production adapter and engine of record. The validation history was:
...
//  replay. ADR 0008 supersedes ADR 0028's earlier primary-engine selection.
//
//  Mapping from Harborline contracts to YDotNet:
...
//  typed cells. If a Harborline consumer needs typed cells, that's a follow-up.
//
//  StubCrdtEngine is retained only as a deterministic test fake. It is not a
//  production fallback.
// ============================================================================
...
namespace Harborline.Api.Kernel.Crdt.Backends;

/// <summary>
/// YDotNet-backed <see cref="ICrdtEngine"/> — Yjs/yrs CRDT with real merge semantics.
/// Paper §2.2 / §9, ADR 0008 engine of record.
/// </summary>
...
internal sealed class YDotNetCrdtDocument : ICrdtDocument
{
    ...
    public YDotNetCrdtDocument(string documentId)
    {
        ...
        // <65k concurrent writers; per paper §2.2 the Harborline federation
...
internal sealed class YDotNetCrdtMap : ICrdtMap
{
    ...
    // Last-known serialized value PER KEY (not just the key set). Tracking values — not presence — lets
    // NotifyExternalChange detect a VALUE-ONLY change to an already-present key (e.g. an inbound delta that
    // renames a contact, or sets its tombstone flag), which a key-set diff alone would silently miss. This
    // mirrors YDotNetCrdtText.FireChange, which has always value-diffed.
    private Dictionary<string, string> _lastValues = new(StringComparer.Ordinal);
    ...
    public void Set<T>(string key, T value)
    {
        ...
        FireChange(key, json, IsDeleted: false);
    }
    ...
    public bool Remove(string key)
    {
        ...
        if (removed) FireChange(key, value: null, IsDeleted: true);
        return removed;
    }
    ...
    {
        var next = SnapshotValues();
        if (Changed is null)
        {
            _lastValues = next;
            return;
        }
        // Added keys AND value-changed keys both raise a non-delete Changed: an inbound delta that renames
        // an already-present contact, or flips its tombstone flag, is a VALUE change with no key-set change,
        // and the downstream read-model reconcile must still run. (Without this, a peer's edit converged in
        // the CRDT doc but never reached the EF read store — earlier repository ticket #1260 F1.)
        foreach (var (k, v) in next)
        {
            if (!_lastValues.TryGetValue(k, out var prev) || !string.Equals(prev, v, StringComparison.Ordinal))
            {
                Changed.Invoke(this, new CrdtMapChangedEventArgs(k, IsDeleted: false));
            }
        }
        foreach (var k in _lastValues.Keys)
        {
    ...
    private Dictionary<string, string> SnapshotValues()
    {
        ...
        foreach (var kvp in iter)
        {
            // Stored cells are JSON-encoded strings (see Set). Read the cell within the SAME read
            // transaction and capture the raw serialized value so a value-only change is detectable; fall
            // back to a presence marker for any non-string cell.
            var cell = _map.Get(tx, kvp.Key);
            values[kvp.Key] = cell is { Tag: OutputTag.String } ? cell.String : string.Empty;
        }
        return values;
    }

    private void FireChange(string key, string? value, bool IsDeleted)
    {
        if (IsDeleted) _lastValues.Remove(key); else _lastValues[key] = value ?? string.Empty;
