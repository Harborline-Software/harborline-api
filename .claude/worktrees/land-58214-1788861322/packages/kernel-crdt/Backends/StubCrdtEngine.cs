// ============================================================================
//  WARNING — TEST-ONLY FAKE, NOT A PRODUCTION CRDT
// ============================================================================
//  ADR 0008 makes YDotNet/yrs the sole production engine of record. This fake
//  remains only for deterministic unit tests that do not exercise real CRDT
//  merge semantics or native loading.
//
//  THIS STUB CONVERGES BY TOTAL-ORDER REPLAY of an op log (not by real CRDT
//  merge semantics). It does NOT provide the tombstone GC mandated by paper §9.
//  DO NOT REGISTER OR SHIP IT IN A PRODUCTION HOST.
// ============================================================================
...
/// <summary>
/// Test-only in-memory fake backend. See file banner. Do not use in production.
/// </summary>
...
internal sealed class StubCrdtMap : ICrdtMap
{
    ...
    private void EmitChanges(Dictionary<string, JsonElement> prev, Dictionary<string, JsonElement> current)
    {
        ...
        // Added keys AND value-changed keys both raise a non-delete Changed (parity with the YDotNet
        // backend). A replay that changes an existing key's value — e.g. an inbound delta renaming a
        // contact, or flipping its tombstone flag — is a value-only change with no key-set change, and the
        // downstream read-model reconcile must still run (earlier repository ticket #1260 F1). Compare serialized text so the
        // diff doesn't depend on JsonElement reference identity.
        foreach (var (key, value) in current)
        {
            if (!prev.TryGetValue(key, out var prevValue)
                || !string.Equals(prevValue.GetRawText(), value.GetRawText(), StringComparison.Ordinal))
            {
