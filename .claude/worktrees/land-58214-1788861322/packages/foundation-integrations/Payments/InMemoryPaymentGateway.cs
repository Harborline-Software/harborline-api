...
public sealed class InMemoryPaymentGateway : IPaymentGateway
{
    ...
    /// <inheritdoc />
    public Task<PaymentVoidResult> VoidAsync(string authorizationHandle, CancellationToken ct)
    {
        ArgumentException.ThrowIfNullOrEmpty(authorizationHandle);
        if (!_journal.TryGetValue(authorizationHandle, out var entry))
        {
            return Task.FromResult(new PaymentVoidResult(PaymentStatus.Failed));
        }
        if (entry.Status == PaymentStatus.Voided)
        {
            // Idempotent — re-void on an already-voided authorization returns Voided.
            return Task.FromResult(new PaymentVoidResult(PaymentStatus.Voided));
        }
        if (entry.Status != PaymentStatus.Authorized)
        {
            // Only an authorized-but-not-captured payment can be voided; a captured
            // payment must be refunded instead.
            return Task.FromResult(new PaymentVoidResult(PaymentStatus.Failed));
        }
        _journal[authorizationHandle] = entry with { Status = PaymentStatus.Voided };
        return Task.FromResult(new PaymentVoidResult(PaymentStatus.Voided));
    }
}
