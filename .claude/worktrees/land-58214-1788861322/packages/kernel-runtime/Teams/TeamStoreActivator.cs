...
public sealed class TeamStoreActivator : ITeamStoreActivator
{
    ...
    private readonly byte[]? _legacyRootSeed;
    ...
    public TeamStoreActivator(
        ...
        : this(factory, derivation, rootSeed, legacyRootSeed: null)
    {
    }

    /// <summary>Create an activator with a one-time legacy-root migration path.</summary>
    /// <param name="factory">Team context factory.</param>
    /// <param name="derivation">SQLCipher key derivation.</param>
    /// <param name="rootSeed">Current recoverable at-rest root.</param>
    /// <param name="legacyRootSeed">
    /// Previous derivation root. When the current key cannot open an existing store, the activator
    /// opens with this key and atomically rotates the store to the current key.
    /// </param>
    public TeamStoreActivator(
        ITeamContextFactory factory,
        ISqlCipherKeyDerivation derivation,
        ...
        byte[]? legacyRootSeed)
    {
        ...
        if (legacyRootSeed is { } legacy)
        {
            if (legacy.Length != 32)
            {
                throw new ArgumentException(
                    $"Legacy root seed must be 32 bytes (was {legacy.Length}).",
                    nameof(legacyRootSeed));
            }
            _legacyRootSeed = legacy.ToArray();
        }
    }
    ...
    private async Task ActivateCoreAsync(TeamId teamId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            catch (InvalidKeyException) when (_legacyRootSeed is not null)
            {
                var legacyKey = _derivation.DeriveSqlCipherKey(
                    _legacyRootSeed,
                    teamId.Value.ToString("D"));
                try
                {
                    await store.OpenAsync(options.DatabasePath, legacyKey, ct).ConfigureAwait(false);
                    await store.RotateKeyAsync(key, ct).ConfigureAwait(false);
                }
                finally
                {
