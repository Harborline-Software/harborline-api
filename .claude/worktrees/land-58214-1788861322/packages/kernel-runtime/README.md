# Harborline.Api.Kernel.Runtime — The Paper's Kernel

`Harborline.Api.Kernel.Runtime` is the runtime kernel described in the foundational
...
It sits **alongside** `Harborline.Api.Kernel` (the type-forwarding facade), per
[ADR 0027 — Kernel Runtime Split](../../docs/adrs/0027-kernel-runtime-split.md).

## Why two packages?

Harborline has two things both honestly called "kernel":

| Package | Responsibility | Shape |
|---|---|---|
| `Harborline.Api.Kernel` | Spec §3 primitive **contracts** (entity store, version store, audit log, permission evaluator, blob store, schema registry, event bus) | Type-forwarding facade over `Harborline.Api.Foundation.*` |
| `Harborline.Api.Kernel.Runtime` | Paper §5.1 **runtime** (node lifecycle, plugin registry, stream topology, projection scheduler, UI-block surfacing) | Live services with state |
...
- **DI:** `AddHarborlineKernelRuntime()` wires the registry + host as singletons.
...
{
    public string Id => "com.harborline.blocks.accounting";
...
- [`Harborline.Api.Kernel` (facade)](../kernel/README.md)
