...
public sealed record JournalEntryLine
{
    ...
    // ---------- ADR 0104 — multi-entity accounting dimensions ----------

    /// <summary>
    /// For inter-company entries only: the counterparty legal entity this line
    /// settles against. Null for ordinary single-entity lines. Lets the
    /// consolidation engine (ADR 0105) pair due-to/due-from clearing across
    /// charts. The entry's own entity is the chart's LegalEntityId; THIS names
    /// the OTHER side.
    /// </summary>
    /// <remarks>
    /// Settlement-pairing LABEL only — it grants NO read access to the
    /// counterparty's ledger (ADR 0104 §3.1, sec-eng 0104-1). Resolving the
    /// counterparty's books still requires that entity's own authorization,
    /// evaluated independently of this label. The consolidation reader (ADR
    /// 0105) resolves its in-scope charts from the <c>ConsolidationScope</c>
    /// ownership graph, NEVER by following this stamp; the label is consumed
    /// only to pair due-to/due-from balances for elimination, never to
    /// discover a chart to read.
    /// </remarks>
    public LegalEntityId? CounterpartyLegalEntityId { get; init; }

    /// <summary>
    /// For multi-member-LLC equity / allocation lines only: the member this capital
    /// transaction or income allocation belongs to. Null for ordinary lines. Lets the
    /// K-1 allocation helper (Wave 3) key per-member allocation off a DIMENSION rather
    /// than off the GL account number, so 1065 special allocations are expressible and a
    /// mis-posted member is detectable. Mirrors the optional typed PropertyId?/TaxCodeId?
    /// dimension precedent; member identity is a reporting axis, not an account encoding.
    /// </summary>
