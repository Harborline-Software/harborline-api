# Harborline.Api.Kernel.Sync

Intra-team gossip anti-entropy daemon for the Harborline local-node architecture.
...
The local-node host also applies an explicit network posture. Its durable
`LocalNode:Sync:NetworkTrust` setting ships as `Unknown`: mDNS does not start
and the pre-trust enrollment opener is closed until an operator selects
`Known`. Already-rostered peers still pass through the unchanged membership
trust gate. This is exposure reduction, not confidentiality: `DELTA_STREAM`
remains unencrypted, so use a controlled private network or an encrypted
overlay for unsealed team, contact, and roster data.
...
| `DependencyInjection/ServiceCollectionExtensions.cs` | `AddHarborlineKernelSync()`, `AddMdnsPeerDiscovery()`, `AddInMemoryPeerDiscovery()`. |
| `tests/` | xUnit harness for the above. |

## Protocol compatibility policy

Named capabilities are the primary compatibility mechanism. An additive
feature is used only when both peers negotiate its stable capability name;
otherwise the session retains the older behavior. Protocol versions are the
backstop for changes that cannot be expressed as capabilities.

`supported_versions` follows an N=2 policy: every release advertises the
current protocol version and its immediate predecessor. The current default is
`1.0.0` with `0.9.0` fallback. The `delta-state-vector` capability is additive:
peers that negotiate it exchange per-document CRDT state vectors on
`GOSSIP_PING`; peers that do not negotiate it receive full-history deltas.
`HELLO` remains the existing six-field message.
...
services.AddHarborlineKernelSync(opts =>
{
...
daemon.AddPeer("/run/harborline/peer-a.sock", peerAPublicKey);
daemon.AddPeer("/run/harborline/peer-b.sock", peerBPublicKey);
...
- **CI / headless containers.** Multicast is typically unavailable; the
  ...
  `HARBORLINE_MDNS_TESTS=1` and is skipped by default.
