...
/// deterministic conversion between Harborline's 32-byte root seed and a 24-word
/// human-readable phrase using the standard BIP-39 English wordlist.
/// </summary>
/// <remarks>
/// <para>
/// <b>What this is.</b> The Harborline keystore stores a 32-byte root seed
...
/// passphrase + PBKDF2 step) — Harborline's keystore handles passphrase
...
public static class PaperKeyDerivation
{
    ...
    /// </summary>
    /// <param name="rootSeed">Harborline root seed (32 bytes). Must not be empty.</param>
    ...
    public static string ToMnemonic(ReadOnlySpan<byte> rootSeed)
    {
        if (rootSeed.Length != EntropyByteLength)
        {
            throw new ArgumentException(
                $"Harborline root seed must be exactly {EntropyByteLength} bytes; got {rootSeed.Length}.",
                nameof(rootSeed));
        }
    ...
    public static byte[] FromMnemonic(string mnemonic)
    {
        ...
        if (words.Length != MnemonicWordCount)
        {
            throw new FormatException(
                $"Harborline paper-key mnemonic must be exactly {MnemonicWordCount} words; got {words.Length}.");
        }
        ...
        if (expectedChecksum[0] != working[EntropyByteLength])
        {
            throw new FormatException(
                ...
                "The phrase is either mistyped or not a valid Harborline paper key.");
        }
    ...
    private static string[] LoadEnglishWordlist()
    {
        ...
        // Embedded as Harborline.Api.Foundation.Recovery.bip39-english.txt
