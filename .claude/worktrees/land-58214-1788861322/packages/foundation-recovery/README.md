# Harborline.Api.Foundation.Recovery
...
Cross-cutting; no business semantics. Consumes kernel-tier crypto primitives (Ed25519, X25519, AES-GCM) from `Harborline.Api.Kernel.Security`.
...
### Stored tenant and subject keys

- **`ITenantKeyProvider`** + `StoredTenantKeyProvider` — random tenant keys wrapped by the resolved at-rest hierarchy; random subject keys wrapped by their tenant key. `ITenantKeyDestroyer` deletes subject or tenant slots. Version-1 root-derived keys are imported lazily for upgrade compatibility.

Deleting a stored subject key makes ciphertext unreadable on rostered replicas that receive the shred.
It does not erase a replica that left the roster before the shred: a stolen, revoked, or disconnected
device can retain an earlier key copy and ciphertext. Revocation prevents future trust; it cannot recall
already-replicated secrets.

### Field-level encryption (W#32 — `Crypto/`)

- **`EncryptedField(Ciphertext, Nonce, KeyVersion)`** — value-record envelope + base64url JSON converter. Stored-key writes use version 2; version 1 remains readable through the lazy legacy-key migration path.
...
There is no mid-state. The `AddHarborlineRecoveryCoordinator()` DI factory throws `InvalidOperationException` at first resolution if exactly one of `IAuditTrail` / `IOperationSigner` is registered. This makes "encryption substrate is wired but audit isn't" a fail-fast misconfiguration rather than a silent log-leak.
...
- [ADR 0049](../../docs/adrs/0049-audit-trail-substrate.md) — audit-emission pattern consumed by the decryptor
