...
- The `RecoveryCoordinator` in `Harborline.Api.Kernel.Security.Recovery` already verifies trustee attestations via `TrusteeAttestation.Verify` before constructing a `KeyRecoveryCompleted` audit record. Records appended via that path are pre-validated.
...
## SoD compensating controls (enrollment Phase C)

This package is the substrate for two of the three **SoD compensating controls** (`project_sod_compensating_controls` — the SoD goal is universal even for a lone operator; the *platform* provides the controls):

1. **Immutable audit trail — the "second set of eyes".** The SoD-significant enrollment operations now have audit-event types (`MemberAdmitted`, `MemberRevoked`, `PermissionsGranted`, `OwnershipTransferred`) and typed payloads (`Payloads/SoDCompensatingControlPayloads.cs`). They land in the SAME `IAuditTrail` the financial posts use — one append-only, signed, tamper-evident, undeletable, `audit:read`-gated surface. The enrollment layer (`foundation-identity-atlas`) owns a kernel-free `ISoDAuditSink` seam; the kernel-audit-backed adapter (`KernelAuditSoDSink`, in `local-node-host`) wires it to this trail. The seam keeps `foundation-identity-atlas` cycle-safe (no kernel-runtime dependency).

2. **Audit-export to an independent reviewer — the "Direct Bank Delivery" control.** `Export/IAuditExportSink` + `Export/AuditExportService` route the audit trail to an outside CPA / reviewer / SIEM, bypassing the sole operator. This is the **audit-stream of the Telemetry & Observability Export category** (`project_one_product_provider_swap_strategy`): a capable bundled default (`NullAuditExportSink` / a local-file writer at the host) behind a provider-swap seam (external SIEM). The audit-stream is held **tighter** than operational telemetry — triggering an export is gated by `telemetry:export`, and the export act is itself recorded (`AuditExported`). Never a hard dependency (fire-and-forget; the local trail is authoritative).

The third control — **CP/AP automated gates** (ADR 0128) — needs no new build here: it is the PBAC permission gating (`gl:post` withheld from the default `member`) plus offline-refusal of CP-class records. Confirmed in `foundation-identity-atlas` tests.
