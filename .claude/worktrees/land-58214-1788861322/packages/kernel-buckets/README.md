# Harborline.Api.Kernel.Buckets

Declarative sync-bucket engine for the Harborline local-node architecture.
...
- **Harborline.Api.Foundation** — shared types.
- **Harborline.Api.UICore** — kernel-facade cross-link.
- **Harborline.Api.Kernel** — kernel-facade cross-link.
- **Harborline.Api.Kernel.EventBus** — `IEventLog` is the substrate buckets filter over.
- **Harborline.Api.Kernel.Security** — `RoleAttestation` / `IAttestationVerifier` drive eligibility.
- **YamlDotNet 16.3.0** — YAML deserialisation.

## Registration

```csharp
services.AddHarborlineKernelBuckets();
