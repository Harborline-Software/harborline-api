...
using Harborline.Api.Kernel.Buckets.LazyFetch;
using Harborline.Api.Kernel.Buckets.Storage;
using Harborline.Api.Kernel.Buckets.Storage.Durability;

namespace Harborline.Api.Kernel.Buckets.DependencyInjection;

/// <summary>
/// Options for <see cref="ServiceCollectionExtensions.AddHarborlineKernelBuckets(IServiceCollection, Action{BucketLoaderOptions})"/>.
...
public sealed class BucketLoaderOptions
{
    ...
    /// <see cref="ServiceCollectionExtensions.AddHarborlineKernelBuckets(IServiceCollection, Action{BucketLoaderOptions})"/>
...
/// <summary>
/// DI extensions for registering the Harborline kernel-buckets surface (paper §10).
/// </summary>
public static class ServiceCollectionExtensions
{
    ...
    /// <see cref="AddHarborlineKernelBuckets(IServiceCollection, Action{BucketLoaderOptions})"/>
    ...
    /// </remarks>
    public static IServiceCollection AddHarborlineKernelBuckets(this IServiceCollection services)
        => AddHarborlineKernelBuckets(services, configure: null);
    ...
    /// <em>eager at first resolution</em>, not at <c>AddHarborlineKernelBuckets</c> call time — so
    ...
    /// </para>
    /// </remarks>
    public static IServiceCollection AddHarborlineKernelBuckets(
        ...
        Action<BucketLoaderOptions>? configure)
    {
        ...
        // PERS-2 (ADR 0137 D5b): the storage-budget manager's eviction is durability-gated, so the guard substrate
        // must be present. Registered fail-closed by default (empty possession ledger ⇒ nothing sheds until a
        // replica is confirmed). The manager is registered via a factory so the guard + optional observer/budget
        // are wired deterministically (avoids constructor-selection ambiguity across its overloads).
        services.AddHarborlineDurabilityGuard();
        services.TryAddSingleton<IStorageBudgetManager>(sp => new InMemoryStorageBudgetManager(
            sp.GetRequiredService<IBucketRegistry>(),
            sp.GetRequiredService<IDurabilityGuard>(),
            sp.GetRequiredService<ILocalReplicaIdentity>(),
            sp.GetService<StorageBudget>() ?? new StorageBudget(),
            sp.GetService<IDurabilityEvictionObserver>() ?? NullDurabilityEvictionObserver.Instance));
